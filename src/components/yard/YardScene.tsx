  ...
  return (
    ...
    >
      ...
      {phlox ? (
        <div
          className="yard-drop absolute"
          style={{ width: roostW, left: "26%", top: "336%" }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/assets/furniture/phloxBank.svg"
            alt=""
            className="pointer-events-none absolute left-1/2 top-[-8%] h-8 w-8 -translate-x-1/2 select-none"
          />
          <FriendSprite
            kit={friendById(phlox.friendId)?.phenotype.artKit ?? "phlox"}
            size={px}
            className={loafClass}
          />
          {bangFriendId === phlox.instanceId ? (
            <button
              type="button"
              onClick={() => onBang?.(phlox.instanceId)}
              className="yard-bang absolute left-1/2 top-0 z-20 grid h-11 w-11 -translate-x-1/2 -translate-y-[72%] place-items-center"
              aria-label={`${phlox.name} has something to say`}
            >
              <UiIcon name="bubble_bang" className="h-7 w-7" />
            </button>
          ) : null}
