...

{
  ...
  console.log("Standalone L4–L9 + L11–L261 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 261) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 261 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 260) {
    throw new Error(`campaign board count must stay 260 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL261) throw new Error("L261 must be the last campaign board");
...

{
  ...
  // ban inventing friend_087+ (Freesia@258 shipped; next gated)
  const srcHit = [
    ...
    return /friend_087/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_087+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 86) throw new Error("TUTORIAL_RESCUES must be 86 (through Freesia)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 86) throw new Error("Met must include through Parsley (42)");
