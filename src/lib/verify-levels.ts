...

{
  ...
  console.log("Standalone L4–L9 + L11–L237 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 237) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 237 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 236) {
    throw new Error(`campaign board count must stay 236 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL237) throw new Error("L237 must be the last campaign board");
...

{
  ...
  // ban inventing friend_079+ (Primrose@234 shipped; next gated)
  const srcHit = [
    ...
    return /friend_079/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_079+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 78) throw new Error("TUTORIAL_RESCUES must be 78 (through Primrose)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 78) throw new Error("Met must include through Parsley (42)");
