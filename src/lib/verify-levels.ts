...

{
  ...
  console.log("Standalone L4–L9 + L11–L246 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 246) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 246 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 245) {
    throw new Error(`campaign board count must stay 245 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL246) throw new Error("L246 must be the last campaign board");
...

{
  ...
  // ban inventing friend_082+ (Clematis@243 shipped; next gated)
  const srcHit = [
    ...
    return /friend_082/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_082+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 81) throw new Error("TUTORIAL_RESCUES must be 81 (through Clematis)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 81) throw new Error("Met must include through Parsley (42)");
