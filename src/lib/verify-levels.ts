...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 267) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 267 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 266) {
    throw new Error(`campaign board count must stay 266 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL267) throw new Error("L267 must be the last campaign board");
...

{
  ...
  // ban inventing friend_089+ (Nasturtium@264 shipped; next gated)
  const srcHit = [
    ...
    return /friend_089/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_089+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 88) throw new Error("TUTORIAL_RESCUES must be 88 (through Nasturtium)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 88) throw new Error("Met must include through Parsley (42)");
