...

{
  ...
  console.log("Standalone L4–L9 + L11–L231 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 231) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 231 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 230) {
    throw new Error(`campaign board count must stay 230 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL231) throw new Error("L231 must be the last campaign board");
...

{
  ...
  // ban inventing friend_077+ (Marigold@228 shipped; next gated)
  const srcHit = [
    ...
    return /friend_077/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_077+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 76) throw new Error("TUTORIAL_RESCUES must be 76 (through Marigold)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 76) throw new Error("Met must include through Parsley (42)");
