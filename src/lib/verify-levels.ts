...

{
  ...
  console.log("Standalone L4–L9 + L11–L243 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 243) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 243 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 242) {
    throw new Error(`campaign board count must stay 242 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL243) throw new Error("L243 must be the last campaign board");
...

{
  ...
  // ban inventing friend_081+ (Cosmos@240 shipped; next gated)
  const srcHit = [
    ...
    return /friend_081/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_081+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 80) throw new Error("TUTORIAL_RESCUES must be 80 (through Cosmos)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 80) throw new Error("Met must include through Parsley (42)");
