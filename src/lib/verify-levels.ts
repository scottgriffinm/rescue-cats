...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 273) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 273 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 272) {
    throw new Error(`campaign board count must stay 272 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL273) throw new Error("L273 must be the last campaign board");
...

{
  ...
  // ban inventing friend_091+ (Pansy@270 shipped; next gated)
  const srcHit = [
    ...
    return /friend_091/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_091+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 90) throw new Error("TUTORIAL_RESCUES must be 90 (through Pansy)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 90) throw new Error("Met must include through Parsley (42)");
