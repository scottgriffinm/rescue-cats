...

{
  ...
  console.log("Standalone L4–L9 + L11–L234 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 234) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 234 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 233) {
    throw new Error(`campaign board count must stay 233 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL234) throw new Error("L234 must be the last campaign board");
...

{
  ...
  // ban inventing friend_078+ (Heather@231 shipped; next gated)
  const srcHit = [
    ...
    return /friend_078/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_078+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 77) throw new Error("TUTORIAL_RESCUES must be 77 (through Heather)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 77) throw new Error("Met must include through Parsley (42)");
