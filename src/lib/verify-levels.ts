...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 288) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 288 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 287) {
    throw new Error(`campaign board count must stay 287 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL288) throw new Error("L288 must be the last campaign board");
...

{
  ...
  // ban inventing friend_096+ (Lantana@285 shipped; next gated)
  const srcHit = [
    ...
    return /friend_096/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_096+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 95) throw new Error("TUTORIAL_RESCUES must be 95 (through Lantana)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 95) throw new Error("Met must include through Parsley (42)");
