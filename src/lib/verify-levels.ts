...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 279) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 279 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 278) {
    throw new Error(`campaign board count must stay 278 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL279) throw new Error("L279 must be the last campaign board");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 92) throw new Error("TUTORIAL_RESCUES must be 92 (through Impatiens)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 92) throw new Error("Met must include through Parsley (42)");
