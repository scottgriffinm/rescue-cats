...

{
  ...
  console.log("Standalone L4–L9 + L11–L252 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 252) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 252 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 251) {
    throw new Error(`campaign board count must stay 251 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL252) throw new Error("L252 must be the last campaign board");
...

{
  ...
  // ban inventing friend_084+ (Anemone@249 shipped; next gated)
  const srcHit = [
    ...
    return /friend_084/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_084+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 83) throw new Error("TUTORIAL_RESCUES must be 83 (through Anemone)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 83) throw new Error("Met must include through Parsley (42)");
