...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 285) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 285 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 284) {
    throw new Error(`campaign board count must stay 284 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL285) throw new Error("L285 must be the last campaign board");
...

{
  ...
  // ban inventing friend_095+ (Calendula@282 shipped; next gated)
  const srcHit = [
    ...
    return /friend_095/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_095+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 94) throw new Error("TUTORIAL_RESCUES must be 94 (through Calendula)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 94) throw new Error("Met must include through Parsley (42)");
