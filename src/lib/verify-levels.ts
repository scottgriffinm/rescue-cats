...

{
  ...
  console.log("Standalone L4–L9 + L11–L258 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 258) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 258 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 257) {
    throw new Error(`campaign board count must stay 257 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL258) throw new Error("L258 must be the last campaign board");
...

{
  ...
  // ban inventing friend_086+ (Ranunculus@255 shipped; next gated)
  const srcHit = [
    ...
    return /friend_086/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_086+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 85) throw new Error("TUTORIAL_RESCUES must be 85 (through Ranunculus)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 85) throw new Error("Met must include through Parsley (42)");
