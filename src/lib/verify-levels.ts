...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 270) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 270 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 269) {
    throw new Error(`campaign board count must stay 269 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL270) throw new Error("L270 must be the last campaign board");
...

{
  ...
  // ban inventing friend_090+ (Petunia@267 shipped; next gated)
  const srcHit = [
    ...
    return /friend_090/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_090+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 89) throw new Error("TUTORIAL_RESCUES must be 89 (through Petunia)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 89) throw new Error("Met must include through Parsley (42)");
