...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 264) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 264 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 263) {
    throw new Error(`campaign board count must stay 263 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL264) throw new Error("L264 must be the last campaign board");
...

{
  ...
  // ban inventing friend_088+ (Geranium@261 shipped; next gated)
  const srcHit = [
    ...
    return /friend_088/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_088+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 87) throw new Error("TUTORIAL_RESCUES must be 87 (through Geranium)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 87) throw new Error("Met must include through Parsley (42)");
