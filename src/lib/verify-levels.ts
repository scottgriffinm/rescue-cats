...

{
  ...
  console.log("Standalone L4–L9 + L11–L249 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 249) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 249 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 248) {
    throw new Error(`campaign board count must stay 248 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL249) throw new Error("L249 must be the last campaign board");
...

{
  ...
  // ban inventing friend_083+ (Wisteria@246 shipped; next gated)
  const srcHit = [
    ...
    return /friend_083/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_083+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 82) throw new Error("TUTORIAL_RESCUES must be 82 (through Wisteria)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 82) throw new Error("Met must include through Parsley (42)");
