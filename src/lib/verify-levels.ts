...

{
  ...
  console.log("Standalone L4–L9 + L11–L168 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 168) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 168 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 167) {
    throw new Error(`campaign board count must stay 167 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL168) throw new Error("L168 must be the last campaign board");
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 168) throw new Error(`HUD denom must be 168, got ${CAMPAIGN_LEVEL_COUNT}`);
  console.log("Level index HUD locks ok · L20=20/168 · L168=168/168 · L9→L11 · L165→L166 · L162→L163 · L159→L160 · L156→L157 · L153→L154 · L150→L151 · L147→L148 · L144→L145 · L141→L142 · L138→L139 · L135→L136 · L132→L133 · L129→L130 · L126→L127 · L123→L124 · L120→L121 · L117→L118 · L114→L115 · L111→L112 · L108→L109 · L105→L106 · L102→L103 · L99→L100 · L96→L97 · L60→L61");
}
...

{
  ...
  // ban inventing friend_056+ (Magnolia@165 shipped; next gated)
  const srcHit = [
    ...
    return /friend_056/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_056+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 55) throw new Error("TUTORIAL_RESCUES must be 55 (through Magnolia)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 55) throw new Error("Met must include through Parsley (42)");
