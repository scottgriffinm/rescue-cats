...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 282) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 282 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 281) {
    throw new Error(`campaign board count must stay 281 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL282) throw new Error("L282 must be the last campaign board");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 93) throw new Error("TUTORIAL_RESCUES must be 93 (through Salvia)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 93) throw new Error("Met must include through Parsley (42)");
