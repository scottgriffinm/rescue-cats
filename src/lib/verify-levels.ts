...

{
  ...
  console.log("Standalone L4–L9 + L11–L225 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 225) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 225 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 224) {
    throw new Error(`campaign board count must stay 224 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL225) throw new Error("L225 must be the last campaign board");
...

{
  ...
  // ban inventing friend_075+ (Bluebell@222 shipped; next gated)
  const srcHit = [
    ...
    return /friend_075/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_075+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 74) throw new Error("TUTORIAL_RESCUES must be 74 (through Bluebell)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 74) throw new Error("Met must include through Parsley (42)");
