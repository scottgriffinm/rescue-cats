...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 291) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 291 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 290) {
    throw new Error(`campaign board count must stay 290 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL291) throw new Error("L291 must be the last campaign board");
...

{
  ...
  // ban inventing friend_097+ (Phlox@288 shipped; next gated)
  const srcHit = [
    ...
    return /friend_097/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_097+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 96) throw new Error("TUTORIAL_RESCUES must be 96 (through Phlox)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 96) throw new Error("Met must include through Parsley (42)");
