...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 276) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 276 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 275) {
    throw new Error(`campaign board count must stay 275 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 276) throw new Error(`HUD denom must be 276, got ${CAMPAIGN_LEVEL_COUNT}`);
...

{
  ...
  // ban inventing friend_092+ (Verbena@273 shipped; next gated)
  const srcHit = [
    ...
    return /friend_092/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_092+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 91) throw new Error("TUTORIAL_RESCUES must be 91 (through Verbena)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 91) throw new Error("Met must include through Parsley (42)");
