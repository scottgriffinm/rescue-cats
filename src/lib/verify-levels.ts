...

{
  ...
  console.log("Standalone L4–L9 + L11–L228 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 228) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 228 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 227) {
    throw new Error(`campaign board count must stay 227 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL228) throw new Error("L228 must be the last campaign board");
...

{
  ...
  // ban inventing friend_076+ (Snapdragon@225 shipped; next gated)
  const srcHit = [
    ...
    return /friend_076/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_076+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 75) throw new Error("TUTORIAL_RESCUES must be 75 (through Snapdragon)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 75) throw new Error("Met must include through Parsley (42)");
