...

{
  ...
  console.log("Standalone L4–L9 + L11–L255 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 255) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 255 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 254) {
    throw new Error(`campaign board count must stay 254 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL255) throw new Error("L255 must be the last campaign board");
...

{
  ...
  // ban inventing friend_085+ (Begonia@252 shipped; next gated)
  const srcHit = [
    ...
    return /friend_085/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_085+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 84) throw new Error("TUTORIAL_RESCUES must be 84 (through Begonia)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 84) throw new Error("Met must include through Parsley (42)");
