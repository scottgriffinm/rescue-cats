...

{
  ...
  console.log("Standalone L4–L9 + L11–L195 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 195) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 195 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 194) {
    throw new Error(`campaign board count must stay 194 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL195) throw new Error("L195 must be the last campaign board");
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 195) throw new Error(`HUD denom must be 195, got ${CAMPAIGN_LEVEL_COUNT}`);
  console.log("Level index HUD locks ok · L20=20/195 · L195=195/195 · L9→L11 · L192→L193 · L189→L190 · L186→L187 · L183→L184 · L180→L181 · L177→L178 · L174→L175 · L171→L172 · L168→L169 · L165→L166 · L162→L163 · L159→L160 · L156→L157 · L153→L154 · L150→L151 · L147→L148 · L144→L145 · L141→L142 · L138→L139 · L135→L136 · L132→L133 · L129→L130 · L126→L127 · L123→L124 · L120→L121 · L117→L118 · L114→L115 · L111→L112 · L108→L109 · L105→L106 · L102→L103 · L99→L100 · L96→L97 · L60→L61");
}
...

{
  ...
  // ban inventing friend_065+ (Iris@192 shipped; next gated)
  const srcHit = [
    ...
    return /friend_065/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_065+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 64) throw new Error("TUTORIAL_RESCUES must be 64 (through Iris)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 64) throw new Error("Met must include through Parsley (42)");
