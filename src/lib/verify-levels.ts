...

{
  ...
  // ban inventing friend_023+ (Coral@66 is friend_022)
  const srcHit = [
  ...
  if (SLICE_UNLOCKS[61] || SLICE_UNLOCKS[69]) {
    throw new Error("no unlocks at 61/69 — Velvet@63 + Coral@66 only past Bean");
  }
...

{
  ...
  if (LEVELS.find((row) => row.id === "L63")?.name !== "Far Peg") throw new Error("L63 untouched");
  if (SLICE_UNLOCKS[69]) throw new Error("no friend_023 yet");
...
{
  // Ch4 Coral@66 (friend_022) — renamed off Mochi
  ...
  if (chipsForFriend("friend_022").includes("Mochi")) throw new Error("Coral must not use Mochi chip");
  if (!chipsForFriend("friend_003").includes("Mochi") && !chipsForFriend("friend_003").length) {
    /* biscuit chips may include Mochi — soft check below */
  }
  const biscuitChips = chipsForFriend("friend_003");
  if (!biscuitChips.includes("Mochi")) throw new Error("Biscuit must keep Mochi chip");
  const m48 = readFileSync(resolve("public/assets/cats/mochi_loaf_48.svg"), "utf8");
  const m72 = readFileSync(resolve("public/assets/cats/mochi_loaf_72.svg"), "utf8");
  if (!m48.includes("#E9B7A6") || !m72.includes("#E9B7A6")) throw new Error("mochi loaf must use #E9B7A6");
  if (!m48.includes("#F6DDD2") || !m72.includes("#F6DDD2")) throw new Error("mochi loaf must use belly #F6DDD2");
  ...
  if (shippedFriendForClear(60)?.friendId !== "friend_020") throw new Error("Bean@60 parade lock");
  if (SLICE_UNLOCKS[69]) throw new Error("no friend_023");
  console.log("Coral @ onClear(66) ok · mochi loaf kit · chips Coral/Bloom/Petal · Biscuit keeps Mochi · Velvet/Bean locked");
}
