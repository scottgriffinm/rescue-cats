...

{
  ...
  console.log("Standalone L4–L9 + L11–L240 + LT02/L06-07/LT08 match locked coords");
}
...

{
  ...
  if (CAMPAIGN_LEVEL_COUNT !== 240) {
    throw new Error(`CAMPAIGN_LEVEL_COUNT must be 240 (max L-id), got ${CAMPAIGN_LEVEL_COUNT}`);
  }
  if (LEVELS.length !== 239) {
    throw new Error(`campaign board count must stay 239 with L10 off-path, got ${LEVELS.length}`);
  }
  for (const level of LEVELS) {
  ...
  if (afterL240) throw new Error("L240 must be the last campaign board");
...

{
  ...
  // ban inventing friend_080+ (Buttercup@237 shipped; next gated)
  const srcHit = [
    ...
    return /friend_080/.test(text) ? [file] : [];
  });
  if (srcHit.length) throw new Error(`friend_080+ must not appear in ${srcHit.join(",")}`);
  if (SLICE_UNLOCKS[61]) {
  ...
  if (TUTORIAL_RESCUES.length !== 79) throw new Error("TUTORIAL_RESCUES must be 79 (through Buttercup)");
...

{
  ...
  if (TUTORIAL_RESCUES.length !== 79) throw new Error("Met must include through Parsley (42)");
