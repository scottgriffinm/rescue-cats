...
export function verifyChapter4Chamomile(solves: Record<string, Array<[string, Dir]>>) {
  for (const [id, name] of [
    ...
    if (/chamomile (cut|gap|stop)/i.test(level.name)) {
      throw new Error(`${id} must not be Chamomile Cut/Gap/Stop`);
    }
    if (/bloom cut|calm gap|bundle stop|nip cut|dream gap|pouch stop|chill cut|frost gap|tin stop|lavender (cut|gap|stop)|catnip (cut|gap|stop)|mint (cut|gap|stop)|needle cut|woody gap|pot stop|rosemary (cut|gap|stop)|pinch cut|twig gap|jar stop|thyme (cut|gap|stop)|marjoram (cut|gap|stop)|softleaf cut|dusty gap|peel stop|oregano (cut|gap|stop)|stone cut|wild gap|pizza stop|tarragon (cut|gap|stop)|spear cut|bitters gap|cruet stop|dill (cut|gap|stop)|seed cut|frondlet gap|ledge stop|parsley (cut|gap|stop)|curl cut|sprig gap|garnish stop/i.test(level.name)) {
  ...
  if (LEVELS.find((row) => row.id === "L154")?.name !== "Bloom Cut") throw new Error("L154 Bloom Cut locked");
  if (LEVELS.find((row) => row.id === "L155")?.name !== "Calm Gap") throw new Error("L155 Calm Gap locked");
  ...
  if (SLICE_UNLOCKS[156] !== "friend_052") throw new Error("SLICE_UNLOCKS[156] must be friend_052");
  if (shippedFriendForClear(156)?.friendId !== "friend_052") throw new Error("onClear(156) must award Chamomile");
  const chamomile = friendById("friend_052");
  if (!chamomile) throw new Error("friend_052 missing from CATALOG");
  if (chamomile.defaultName !== "Chamomile") throw new Error("default name must be Chamomile");
  if (chamomile.unlockClear !== 156) throw new Error("Chamomile unlockClear must be 156");
  if (chamomile.phenotype.artKit !== "chamomile") throw new Error("Chamomile artKit must be chamomile");
  if (chamomile.phenotype.personality !== "Cup-soft") throw new Error("Chamomile personality must be Cup-soft");
  if (chamomile.phenotype.boardColor !== "gray") throw new Error("Chamomile boardColor must be gray");
  if (chamomile.phenotype.color !== "Gray") throw new Error("Chamomile color must be Gray");
  if (chamomile.phenotype.pattern !== "Freckled") throw new Error("Chamomile pattern must be Freckled");
  if (chipsForFriend("friend_052").join(",") !== "Chamomile,Daisy,Tea") {
    throw new Error(`Chamomile chips must be Chamomile/Daisy/Tea, got ${chipsForFriend("friend_052").join(",")}`);
  }
  if (chipsForFriend("friend_052").some((c) => /lavender|^bloom$|^calm$|catnip|^nip$|^dream$|mint|chill|^frost$|rosemary|needle|^woody$|thyme|pinch|^twig$|marjoram|softleaf|^peel$|oregano|wild|^bunch$|tarragon|spear|bitters|dill|frondlet|^seed$|parsley|curl|sprig|lovage|stem|rib|chervil|frill|lace|fennel|^frond$|anise|pebble|^sage$/i.test(c))) {
    throw new Error("Chamomile chips must ban Lavender/Bloom/Calm/Catnip/Nip/Dream/Mint/Chill/Frost/Rosemary/Needle/Woody/Thyme/Pinch/Twig/Marjoram/Softleaf/Peel/Oregano/Wild/Bunch/Tarragon/Spear/Bitters/Dill/Frondlet/Seed/Parsley/Curl/Sprig/Lovage/Chervil/Fennel/Frond/Pebble/Sage");
  }
  const chamomile48 = readFileSync(resolve("public/assets/cats/chamomile_loaf_48.svg"), "utf8");
  const chamomile72 = readFileSync(resolve("public/assets/cats/chamomile_loaf_72.svg"), "utf8");
  if (!chamomile48.includes("#E8D5A3") || !chamomile72.includes("#E8D5A3")) throw new Error("Chamomile loaf must use soft cream-gold coat #E8D5A3");
  if (!chamomile48.includes("#8A6B2E") || !chamomile72.includes("#8A6B2E")) throw new Error("Chamomile loaf must use tiny petal freckles #8A6B2E");
  if (!chamomile48.includes("#F7E8C4") || !chamomile72.includes("#F7E8C4")) throw new Error("Chamomile loaf must use belly #F7E8C4");
  for (const hex of ["#B8A0C8", "#5A3F6E", "#EDE4F4", "#8A6AA8", "#8FBF9A", "#2F5C3A", "#DCEFE2", "#7EC8A3", "#2F6B52", "#D4F4E8", "#7A9B88", "#2F463C", "#C8D6CE", "#A3B57C", "#4E5C36", "#D2D8B0", "#8FA86A", "#3F5230", "#D8D4A8", "#7A9A4E", "#4A5C2E", "#6B8F4E", "#2F4A28", "#C4D6A4", "#8FBF7A", "#4A6B3E", "#6FA86A", "#3F6B3C", "#9CB87A", "#5E7348", "#C6D9B4", "#5E7F52", "#E6C86E", "#7A9B6A", "#6A7D6E", "#1A2C24", "#7E8F86", "#C8D24A"]) {
    if (chamomile48.includes(hex) || chamomile72.includes(hex)) throw new Error(`Chamomile loaf must not use prior herb coat ${hex}`);
  }
  if (furnitureGiftsForClear(156).length) throw new Error("Chamomile@156 must gift no furniture");
  ...
  if (TUTORIAL_RESCUES.length !== 95) throw new Error("Met must include through Chamomile (52)");
  ...
  const l157 = LEVELS.find((r) => r.id === "L157")!;
  const l158 = LEVELS.find((r) => r.id === "L158")!;
  const l159 = LEVELS.find((r) => r.id === "L159")!;
  if (/bloom|calm|bundle|nip|dream|pouch|chill|frost|tin|needle|woody|pot|pinch|twig|jar|softleaf|dusty|peel|stone|wild|pizza|spear|bitters|cruet|seed|frondlet|ledge|garnish|bed/i.test([l157.name, l158.name, l159.name].join(","))) {
    throw new Error("L157–L159 must not reuse Bloom/Calm/Bundle/Nip/Dream/Pouch/Chill/Frost/Tin/Needle/Woody/Pot/Pinch/Twig/Jar/Softleaf/Dusty/Peel/Stone/Wild/Pizza/Spear/Bitters/Cruet/Seed/Frondlet/Ledge/Garnish/Bed names");
  }
  const chamomileGatePair = (level: typeof l157) => level.gates.map((g) => `${g.x},${g.y}`).sort().join("/");
  if (chamomileGatePair(l157) !== "4,1/4,3") throw new Error(`L157 gates must be delta (0,2) pair, got ${chamomileGatePair(l157)}`);
  if (chamomileGatePair(l158) !== "0,1/3,2") throw new Error(`L158 gates must be delta (3,1) pair, got ${chamomileGatePair(l158)}`);
  if (chamomileGatePair(l159) !== "1,5/3,1") throw new Error(`L159 gates must be delta (2,-4) pair, got ${chamomileGatePair(l159)}`);
  const occ159 = new Set([
    ...l159.cats.map((c) => `${c.x},${c.y}`),
    ...l159.gates.map((g) => `${g.x},${g.y}`),
  ]);
  if (occ159.has("5,5")) throw new Error("L159 Cup must be off Nori seat (5,5)");
  if (occ159.has("4,2") || occ159.has("4,0") || occ159.has("4,1") || occ159.has("0,4")) {
    throw new Error("L159 Cup must be off Pouch seats");
  }
  if (occ159.has("0,3") || occ159.has("3,5")) throw new Error("L159 Cup must be off Tin Stop seats");
  if (occ159.has("3,4") || occ159.has("4,3")) throw new Error("L159 Cup must be off Tin Stop cats");
  if (occ159.has("1,3") || occ159.has("2,2")) throw new Error("L159 Cup must be off Pot Stop seats");
  if (occ159.has("2,3") || occ159.has("3,3")) throw new Error("L159 Cup must be off Pot Stop cats");
  if (occ159.has("3,2") || occ159.has("5,4")) throw new Error("L159 Cup must be off Jar Stop seats");
  if (occ159.has("1,0") || occ159.has("3,0")) throw new Error("L159 Cup must be off Jar Stop cats");
  if (occ159.has("2,4") || occ159.has("1,1") || occ159.has("2,1") || occ159.has("4,5")) {
    throw new Error("L159 Cup must be off Bundle Stop seats");
  }
  if (l159.gates[0].y === l159.gates[1].y) throw new Error("L159 Cup must not same-row Nest");
  if (l159.gates[0].x === l159.gates[1].x) throw new Error("L159 Cup must not column Porch");
  if (l157.cats[0].x === l157.cats[1].x) throw new Error("L157 Daisy must not stacked-column Vine");
  if (solves.L157[0][0] === "cat_orange" && solves.L157[0][1] === "s") {
    throw new Error("L157 Daisy Cut must not open orange-south (Bloom clone)");
  }
  if (solves.L157[0][0] === "cat_gray" && solves.L157[0][1] === "s") {
    throw new Error("L157 Daisy Cut must not open gray-south (Nip clone)");
  }
  if (solves.L157[0][0] === "cat_gray" && solves.L157[0][1] === "n") {
    throw new Error("L157 Daisy Cut must not open gray-north (Chill clone)");
  }
  if (solves.L158[0][0] === "cat_black" && solves.L158[0][1] === "w") {
    throw new Error("L158 Tea Gap must not open black-west (Calm clone)");
  }
  if (solves.L158[0][0] === "cat_orange" && solves.L158[0][1] === "n") {
    throw new Error("L158 Tea Gap must not open orange-north (Dream clone)");
  }
  if (solves.L158[0][0] === "cat_orange" && solves.L158[0][1] === "e") {
    throw new Error("L158 Tea Gap must not open orange-east (Frost clone)");
  }
  for (const level of [l157, l158, l159]) {
  ...
  const wallKeyC = (level: typeof l157) =>
    [...level.walls].map((w) => `${w.x},${w.y}`).sort().join(";");
  const priorWallsC = ["L154", "L155", "L156", "L151", "L152", "L153", "L148", "L149", "L150", "L145", "L146", "L147", "L142", "L143", "L144", "L139", "L140", "L141"].map((id) => wallKeyC(LEVELS.find((r) => r.id === id)!));
  for (const id of ["L157", "L158", "L159"] as const) {
    const key = wallKeyC(LEVELS.find((r) => r.id === id)!);
    if (priorWallsC.includes(key)) throw new Error(`${id} wall twin of L139–L156`);
  }
  if (ART_KIT_PATH.chamomile.loaf48 !== "/assets/cats/chamomile_loaf_48.svg") throw new Error("ART_KIT_PATH.chamomile loaf48");
  if (ART_KIT_PATH.chamomile.loaf72 !== "/assets/cats/chamomile_loaf_72.svg") throw new Error("ART_KIT_PATH.chamomile loaf72");
  if (ART_KIT_PATH.lavender.loaf48 !== "/assets/cats/lavender_loaf_48.svg") throw new Error("Lavender kit must stay");
  const saveC = readFileSync(resolve("src/components/providers/SaveProvider.tsx"), "utf8");
  if (!saveC.includes("chamomile cup")) throw new Error("SaveProvider must yard-bubble chamomile cup for Chamomile");
  if (!existsSync(resolve("data/collection/CH4_FRIEND_052.md"))) throw new Error("missing CH4_FRIEND_052.md");
  if (!existsSync(resolve("data/collection/chapter4_chamomile_bang.json"))) throw new Error("missing collection chamomile bang");
  if (!existsSync(resolve("data/chapter4_chamomile_bang.json"))) throw new Error("missing chapter4_chamomile_bang.json");
  const nameModalC = readFileSync(resolve("src/components/puzzle/NameCatModal.tsx"), "utf8");
  if (!nameModalC.includes("CHAMOMILE_FRIEND_ID")) throw new Error("NameCatModal must lockChips Chamomile");
  if (!nameModalC.includes("LAVENDER_FRIEND_ID")) throw new Error("NameCatModal must keep lockChips Lavender");
  ...
  console.log("Ch4 Chamomile@156 + L157–L159 ok · chips Chamomile/Daisy/Tea · coat #E8D5A3 + tiny petal freckles #8A6B2E · Daisy Cut / Tea Gap / Cup Stop · Lavender/Catnip/Mint/Rosemary/Thyme/Marjoram/Oregano/Tarragon/Dill/Parsley/Lovage/Chervil/Fennel/Sorrel/Nettle/Ivy/Briar/Thistle/Plum/Fig/Basil/Clay/Ivory/Juniper/Linen/Cocoa/Steve/Maple/Blue/Coral/Velvet/Bean locked");
}
