...
export function verifyChapter4Azalea(solves: Record<string, Array<[string, Dir]>>) {
  ...
  if (SLICE_UNLOCKS[180] !== "friend_060") throw new Error("SLICE_UNLOCKS[180] must be friend_060");
  if (shippedFriendForClear(180)?.friendId !== "friend_060") throw new Error("onClear(180) must award Azalea");
  const azalea = friendById("friend_060");
  if (!azalea) throw new Error("friend_060 missing from CATALOG");
  if (azalea.defaultName !== "Azalea") throw new Error("default name must be Azalea");
  if (azalea.unlockClear !== 180) throw new Error("Azalea unlockClear must be 180");
  if (azalea.phenotype.artKit !== "azalea") throw new Error("Azalea artKit must be azalea");
  if (azalea.phenotype.personality !== "Planter-soft") throw new Error("Azalea personality must be Planter-soft");
  if (azalea.phenotype.boardColor !== "orange") throw new Error("Azalea boardColor must be orange");
  if (azalea.phenotype.color !== "Orange") throw new Error("Azalea color must be Orange");
  if (azalea.phenotype.pattern !== "Freckled") throw new Error("Azalea pattern must be Freckled");
  if (chipsForFriend("friend_060").join(",") !== "Azalea,Fizz,Flare") {
    throw new Error(`Azalea chips must be Azalea/Fizz/Flare, got ${chipsForFriend("friend_060").join(",")}`);
  }
  if (chipsForFriend("friend_060").some((c) => /peony|^bud$|^satin$|camellia|^wax$|^rose$|gardenia|^snow$|^velvet$|hibiscus|^roselle$|^punch$|magnolia|^cream$|^blush$|jasmine|^blossom$|^honey$|bergamot|^citrus$|^earl$|chamomile|^daisy$|^tea$|lavender|^bloom$|^calm$|catnip|^nip$|^dream$|mint|chill|^frost$|^ivory$|^lace$|^sheer$|rosemary|needle|^woody$|thyme|pinch|^twig$|marjoram|softleaf|^peel$|oregano|wild|^bunch$|tarragon|spear|bitters|dill|frondlet|^seed$|parsley|curl|sprig|lovage|stem|rib|chervil|frill|lace|fennel|^frond$|anise|pebble|^sage$/i.test(c))) {
    throw new Error("Azalea chips must ban Peony/Bud/Satin/Camellia/Wax/Rose/Gardenia/Snow/Velvet/Hibiscus/Roselle/Punch/Magnolia/Cream/Blush/Jasmine/Blossom/Honey and all prior pools/Pebble/Sage");
  }
  const azalea48 = readFileSync(resolve("public/assets/cats/azalea_loaf_48.svg"), "utf8");
  const azalea72 = readFileSync(resolve("public/assets/cats/azalea_loaf_72.svg"), "utf8");
  if (!azalea48.includes("#B84A8C") || !azalea72.includes("#B84A8C")) throw new Error("Azalea loaf must use vivid fuchsia-magenta coat #B84A8C");
  if (!azalea48.includes("#3F1830") || !azalea72.includes("#3F1830")) throw new Error("Azalea loaf must use pollen freckles #3F1830");
  if (!azalea48.includes("#E8A0C4") || !azalea72.includes("#E8A0C4")) throw new Error("Azalea loaf must use belly #E8A0C4");
  for (const hex of ["#E8B4C8", "#5A3048", "#F7DCE6", "#C4789A", "#C45A6A", "#5A2030", "#F3C8CC", "#8E3848", "#F6F1E8", "#4F6B4A", "#FFFBF3", "#C5C0B2", "#D46A8A", "#5A1F3A", "#C24A6E", "#EBB0C2", "#F7E8D2", "#C48A7A", "#FFF4E4", "#E8B8A4", "#F2D4C4", "#6E3A42", "#F4EFE6", "#7A6B4E", "#FBF7EE", "#F0C98A", "#9A5A1A", "#FFF1C4", "#E8D5A3", "#8A6B2E", "#F7E8C4", "#C4A66A", "#B8A0C8", "#5A3F6E", "#EDE4F4", "#8A6AA8", "#8FBF9A", "#2F5C3A", "#DCEFE2", "#7EC8A3", "#2F6B52", "#D4F4E8", "#7A9B88", "#2F463C", "#C8D6CE", "#A3B57C", "#4E5C36", "#D2D8B0", "#8FA86A", "#3F5230", "#D8D4A8", "#7A9A4E", "#4A5C2E", "#6B8F4E", "#2F4A28", "#C4D6A4", "#8FBF7A", "#4A6B3E", "#6FA86A", "#3F6B3C", "#9CB87A", "#5E7348", "#C6D9B4", "#5E7F52", "#E6C86E", "#7A9B6A", "#6A7D6E", "#1A2C24", "#7E8F86", "#C8D24A", "#EBE3C4", "#C9C09A", "#F3EBD0"]) {
    if (azalea48.includes(hex) || azalea72.includes(hex)) throw new Error(`Azalea loaf must not use prior herb coat ${hex}`);
  }
  if (furnitureGiftsForClear(180).length) throw new Error("Azalea@180 must gift no furniture");
  ...
  if (TUTORIAL_RESCUES.length !== 60) throw new Error("Met must include through Azalea (60)");
  ...
  if (SLICE_UNLOCKS[183]) throw new Error("no friend_061 @183 this slice");
  ...
  const l181 = LEVELS.find((r) => r.id === "L181")!;
  const l182 = LEVELS.find((r) => r.id === "L182")!;
  const l183 = LEVELS.find((r) => r.id === "L183")!;
  if (/\b(wax|rose|tray|snow|velvet|dish|drift|latch|claim|roselle|punch|sip|hibiscus|cream|blush|magnolia|blossom|honey|bloom|citrus|earl|saucer|daisy|tea|cup|calm|bundle|nip|dream|pouch|chill|frost|tin|needle|woody|pot|pinch|twig|jar|softleaf|dusty|peel|stone|wild|pizza|spear|bitters|cruet|seed|frondlet|ledge|garnish|bed|gardenia|camellia|peony|bud|satin|bowl)\b/i.test([l181.name, l182.name].join(","))) {
    throw new Error("L181–L182 must not reuse Wax/Rose/Tray/Snow/Velvet/Dish/Roselle/Punch/Sip/Cream/Blush/Cup/Bloom/Saucer/Peony/Bud/Satin/Bowl and prior triad names");
  }
  const azaleaGatePair = (level: typeof l181) => level.gates.map((g) => `${g.x},${g.y}`).sort().join("/");
  if (azaleaGatePair(l181) !== "2,0/3,4") throw new Error(`L181 gates must be delta (-1, -4) pair, got ${azaleaGatePair(l181)}`);
  if (azaleaGatePair(l182) !== "0,3/1,4") throw new Error(`L182 gates must be delta (-1, -1) pair, got ${azaleaGatePair(l182)}`);
  if (azaleaGatePair(l183) !== "3,5/4,4") throw new Error(`L183 gates must be delta (-1, 1) pair, got ${azaleaGatePair(l183)}`);
  const occ183 = new Set([
    ...l183.cats.map((c) => `${c.x},${c.y}`),
    ...l183.gates.map((g) => `${g.x},${g.y}`),
  ]);
  if (occ183.has("5,5")) throw new Error("L183 Planter must be off Nori seat (5,5)");
  if (occ183.has("4,0") || occ183.has("3,4") || occ183.has("1,0") || occ183.has("4,1")) {
    throw new Error("L183 Planter must be off Tray seats");
  }
  if (occ183.has("2,1") || occ183.has("3,0") || occ183.has("1,1") || occ183.has("0,0")) {
    throw new Error("L183 Planter must be off Dish seats");
  }
  if (occ183.has("2,2") || occ183.has("5,4") || occ183.has("2,3") || occ183.has("0,2")) {
    throw new Error("L183 Planter must be off Sip seats");
  }
  if (occ183.has("0,5") || occ183.has("5,0") || occ183.has("1,4") || occ183.has("5,1")) {
    throw new Error("L183 Planter must be off Bloom seats");
  }
  if (occ183.has("2,0") || occ183.has("2,5") || occ183.has("5,3") || occ183.has("1,2")) {
    throw new Error("L183 Planter must be off Saucer seats");
  }
  if (occ183.has("3,2") || occ183.has("3,3") || occ183.has("0,1") || occ183.has("1,3")) {
    throw new Error("L183 Planter must be off L168 Bowl seats");
  }
  if (occ183.has("0,3") || occ183.has("4,2") || occ183.has("4,3") || occ183.has("0,4")) {
    throw new Error("L183 Planter must be off L180 Bowl seats");
  }
  if (l183.gates[0].y === l183.gates[1].y) throw new Error("L183 Planter must not same-row Nest");
  if (l183.gates[0].x === l183.gates[1].x) throw new Error("L183 Planter must not column Porch");
  if (l181.cats[0].x === l181.cats[1].x) throw new Error("L181 Fizz must not stacked-column Vine");
  if (solves.L181[0][0] === "cat_orange" && solves.L181[0][1] === "e") {
    throw new Error("L181 Fizz Cut must not open orange-east (Bud Cut clone)");
  }
  if (solves.L181[0][0] === "cat_gray" && solves.L181[0][1] === "n") {
    throw new Error("L181 Fizz Cut must not open gray-north (Wax Cut clone)");
  }
  if (solves.L181[0][0] === "cat_gray" && solves.L181[0][1] === "w") {
    throw new Error("L181 Fizz Cut must not open gray-west (Snow Drift clone)");
  }
  if (solves.L181[0][0] === "cat_gray" && solves.L181[0][1] === "e") {
    throw new Error("L181 Fizz Cut must not open gray-east (Roselle clone)");
  }
  if (solves.L182[0][0] === "cat_black" && solves.L182[0][1] === "e") {
    throw new Error("L182 Flare Gap must not open black-east (Satin Gap clone)");
  }
  if (solves.L182[0][0] === "cat_orange" && solves.L182[0][1] === "e") {
    throw new Error("L182 Flare Gap must not open orange-east (Rose Gap clone)");
  }
  if (solves.L182[0][0] === "cat_black" && solves.L182[0][1] === "n") {
    throw new Error("L182 Flare Gap must not open black-north (Velvet Latch clone)");
  }
  if (solves.L182[0][0] === "cat_orange" && solves.L182[0][1] === "s") {
    throw new Error("L182 Flare Gap must not open orange-south (Punch clone)");
  }
  if (solves.L183[0][0] === "cat_gray" && solves.L183[0][1] === "e") {
    throw new Error("L183 Planter Stop must not open gray-east (Bowl Stop clone)");
  }
  if (solves.L183[0][0] === "cat_orange" && solves.L183[0][1] === "w") {
    throw new Error("L183 Planter Stop must not open orange-west (Tray Stop clone)");
  }
  if (solves.L183[0][0] === "cat_gray" && solves.L183[0][1] === "s") {
    throw new Error("L183 Planter Stop must not open gray-south (Dish Claim clone)");
  }
  if (solves.L183[0][0] === "cat_orange" && solves.L183[0][1] === "e") {
    throw new Error("L183 Planter Stop must not open orange-east (Sip clone)");
  }
  for (const level of [l181, l182, l183]) {
  ...
  const wallKeyA = (level: typeof l181) =>
    [...level.walls].map((w) => `${w.x},${w.y}`).sort().join(";");
  const priorWallsA = ["L178", "L179", "L180", "L175", "L176", "L177", "L172", "L173", "L174", "L169", "L170", "L171", "L166", "L167", "L168", "L163", "L164", "L165", "L160", "L161", "L162", "L157", "L158", "L159", "L154", "L155", "L156", "L151", "L152", "L153", "L148", "L149", "L150", "L145", "L146", "L147", "L142", "L143", "L144", "L139", "L140", "L141"].map((id) => wallKeyA(LEVELS.find((r) => r.id === id)!));
  for (const id of ["L181", "L182", "L183"] as const) {
    const key = wallKeyA(LEVELS.find((r) => r.id === id)!);
    if (priorWallsA.includes(key)) throw new Error(`${id} wall twin of L139–L180`);
  }
  if (ART_KIT_PATH.azalea.loaf48 !== "/assets/cats/azalea_loaf_48.svg") throw new Error("ART_KIT_PATH.azalea loaf48");
  if (ART_KIT_PATH.azalea.loaf72 !== "/assets/cats/azalea_loaf_72.svg") throw new Error("ART_KIT_PATH.azalea loaf72");
  ...
  if (!saveA.includes("azalea planter")) throw new Error("SaveProvider must yard-bubble azalea planter for Azalea");
  if (!existsSync(resolve("data/collection/CH4_FRIEND_060.md"))) throw new Error("missing CH4_FRIEND_060.md");
  if (!existsSync(resolve("data/collection/chapter4_azalea_bang.json"))) throw new Error("missing collection azalea bang");
  if (!existsSync(resolve("data/chapter4_azalea_bang.json"))) throw new Error("missing chapter4_azalea_bang.json");
  const nameModalA = readFileSync(resolve("src/components/puzzle/NameCatModal.tsx"), "utf8");
  if (!nameModalA.includes("AZALEA_FRIEND_ID")) throw new Error("NameCatModal must lockChips Azalea");
  if (!nameModalA.includes("PEONY_FRIEND_ID")) throw new Error("NameCatModal must keep lockChips Peony");
  ...
  console.log("Ch4 Azalea@180 + L181–L183 ok · chips Azalea/Fizz/Flare · coat #B84A8C + pollen freckles #3F1830 · Fizz Cut / Flare Gap / Planter Stop ·  locked");
}
