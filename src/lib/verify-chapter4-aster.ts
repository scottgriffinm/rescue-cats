...
export function verifyChapter4Aster(solves: Record<string, Array<[string, Dir]>>) {
  ...
  if (SLICE_UNLOCKS[189] !== "friend_063") throw new Error("SLICE_UNLOCKS[189] must be friend_063");
  if (shippedFriendForClear(189)?.friendId !== "friend_063") throw new Error("onClear(189) must award Aster");
  const aster = friendById("friend_063");
  if (!aster) throw new Error("friend_063 missing from CATALOG");
  if (aster.defaultName !== "Aster") throw new Error("default name must be Aster");
  if (aster.unlockClear !== 189) throw new Error("Aster unlockClear must be 189");
  if (aster.phenotype.artKit !== "aster") throw new Error("Aster artKit must be aster");
  if (aster.phenotype.personality !== "Aster-soft") throw new Error("Aster personality must be Aster-soft");
  if (aster.phenotype.boardColor !== "gray") throw new Error("Aster boardColor must be gray");
  if (aster.phenotype.color !== "Gray") throw new Error("Aster color must be Gray");
  if (aster.phenotype.pattern !== "Freckled") throw new Error("Aster pattern must be Freckled");
  if (chipsForFriend("friend_063").join(",") !== "Aster,Petal,Drift") {
    throw new Error(`Aster chips must be Aster/Petal/Drift, got ${chipsForFriend("friend_063").join(",")}`);
  }
  if (chipsForFriend("friend_063").some((c) => /zinnia|^quill$|^gleam$|dahlia|^spire$|^ember$|azalea|^fizz$|^flare$|peony|^bud$|^satin$|camellia|^wax$|^rose$|gardenia|^snow$|^velvet$|hibiscus|^roselle$|^punch$|magnolia|^cream$|^blush$|jasmine|^blossom$|^honey$|bergamot|^citrus$|^earl$|chamomile|^daisy$|^tea$|lavender|^bloom$|^calm$|catnip|^nip$|^dream$|mint|chill|^frost$|^ivory$|^lace$|^sheer$|rosemary|needle|^woody$|thyme|pinch|^twig$|marjoram|softleaf|^peel$|oregano|wild|^bunch$|tarragon|spear|bitters|dill|frondlet|^seed$|parsley|curl|sprig|lovage|stem|rib|chervil|frill|lace|fennel|^frond$|anise|pebble|^sage$/i.test(c))) {
    throw new Error("Aster chips must ban Zinnia/Quill/Gleam/Dahlia/Spire/Ember/Azalea/Fizz/Flare/Peony/Bud/Satin/Camellia/Wax/Rose/Gardenia/Snow/Velvet/Hibiscus/Roselle/Punch/Magnolia/Cream/Blush/Jasmine/Blossom/Honey and all prior pools/Pebble/Sage");
  }
  const aster48 = readFileSync(resolve("public/assets/cats/aster_loaf_48.svg"), "utf8");
  const aster72 = readFileSync(resolve("public/assets/cats/aster_loaf_72.svg"), "utf8");
  if (!aster48.includes("#7B6BB5") || !aster72.includes("#7B6BB5")) throw new Error("Aster loaf must use cool periwinkle-lilac coat #7B6BB5");
  if (!aster48.includes("#2A2040") || !aster72.includes("#2A2040")) throw new Error("Aster loaf must use dusk freckles #2A2040");
  if (!aster48.includes("#C8B8EE") || !aster72.includes("#C8B8EE")) throw new Error("Aster loaf must use belly #C8B8EE");
  for (const hex of ["#E85A2A", "#3A1808", "#F4A06A", "#B33A14", "#8B2E4A", "#2A1018", "#C86A80", "#5C1832", "#B84A8C", "#3F1830", "#E8A0C4", "#7A2458", "#E8B4C8", "#5A3048", "#F7DCE6", "#C4789A", "#C45A6A", "#5A2030", "#F3C8CC", "#8E3848", "#F6F1E8", "#4F6B4A", "#FFFBF3", "#C5C0B2", "#D46A8A", "#5A1F3A", "#C24A6E", "#EBB0C2", "#F7E8D2", "#C48A7A", "#FFF4E4", "#E8B8A4", "#F2D4C4", "#6E3A42", "#F4EFE6", "#7A6B4E", "#FBF7EE", "#F0C98A", "#9A5A1A", "#FFF1C4", "#E8D5A3", "#8A6B2E", "#F7E8C4", "#C4A66A", "#B8A0C8", "#5A3F6E", "#EDE4F4", "#8A6AA8", "#8FBF9A", "#2F5C3A", "#DCEFE2", "#7EC8A3", "#2F6B52", "#D4F4E8", "#7A9B88", "#2F463C", "#C8D6CE", "#A3B57C", "#4E5C36", "#D2D8B0", "#8FA86A", "#3F5230", "#D8D4A8", "#7A9A4E", "#4A5C2E", "#6B8F4E", "#2F4A28", "#C4D6A4", "#8FBF7A", "#4A6B3E", "#6FA86A", "#3F6B3C", "#9CB87A", "#5E7348", "#C6D9B4", "#5E7F52", "#E6C86E", "#7A9B6A", "#6A7D6E", "#1A2C24", "#7E8F86", "#C8D24A", "#EBE3C4", "#C9C09A", "#F3EBD0"]) {
    if (aster48.includes(hex) || aster72.includes(hex)) throw new Error(`Aster loaf must not use prior herb coat ${hex}`);
  }
  if (furnitureGiftsForClear(189).length) throw new Error("Aster@189 must gift no furniture");
  ...
  if (TUTORIAL_RESCUES.length !== 63) throw new Error("Met must include through Aster (63)");
  ...
  if (SLICE_UNLOCKS[192]) throw new Error("no friend_064 @192 this slice");
  if (/pebble|Pebble/i.test(aster48 + aster72)) throw new Error("Aster art must not use Pebble");
  if (/zinnia|quill|gleam|dahlia|spire|ember|azalea|fizz|flare|peony|camellia|wax|gardenia|hibiscus|roselle|punch|magnolia|cream|blush|jasmine|blossom|honey|bergamot|citrus|earl|chamomile|daisy|tea|lavender|bloom|calm|catnip|mint|chill|frost|ivory|sheer|rosemary|needle|woody|thyme|pinch|twig|marjoram|softleaf|peel|oregano|wild|bunch|tarragon|spear|bitters|dill|parsley|curl|sprig|lovage|stem|rib|chervil|frill|lace|fennel|anise/i.test(aster48 + aster72)) {
    throw new Error("Aster art must not collide Zinnia/Dahlia/Azalea/Peony/Camellia/Gardenia/Hibiscus/Magnolia/Jasmine/Bergamot/Chamomile/Lavender/Catnip/Mint/Ivory/Rosemary/Thyme/Marjoram/Oregano/Tarragon/Dill/Parsley/Lovage/Chervil/Fennel marks");
  }
  const l190 = LEVELS.find((r) => r.id === "L190")!;
  const l191 = LEVELS.find((r) => r.id === "L191")!;
  const l192 = LEVELS.find((r) => r.id === "L192")!;
  if (/\b(quill|gleam|urn|zinnia|spire|ember|vase|dahlia|fizz|flare|planter|azalea|wax|rose|tray|snow|velvet|latch|claim|roselle|punch|sip|hibiscus|cream|blush|magnolia|blossom|honey|bloom|citrus|earl|saucer|daisy|tea|cup|calm|bundle|nip|dream|pouch|chill|frost|tin|needle|woody|pot|pinch|twig|jar|softleaf|dusty|peel|stone|wild|pizza|spear|bitters|cruet|seed|frondlet|ledge|garnish|bed|gardenia|camellia|peony|bud|satin|bowl)\b/i.test([l190.name, l191.name].join(","))) {
    throw new Error("L190–L191 must not reuse Quill/Gleam/Urn/Zinnia/Spire/Ember/Vase/Dahlia/Fizz/Flare/Planter/Azalea/Wax/Rose/Tray/Snow/Velvet/Roselle/Punch/Sip/Cream/Blush/Cup/Bloom/Saucer/Peony/Bud/Satin/Bowl and prior triad names");
  }
  const asterGatePair = (level: typeof l190) => level.gates.map((g) => `${g.x},${g.y}`).sort().join("/");
  if (asterGatePair(l190) !== "0,0/1,4") throw new Error(`L190 gates must be delta (-1, -4) pair, got ${asterGatePair(l190)}`);
  if (asterGatePair(l191) !== "1,1/3,0") throw new Error(`L191 gates must be delta (2, -1) pair, got ${asterGatePair(l191)}`);
  if (asterGatePair(l192) !== "2,1/3,0") throw new Error(`L192 gates must be delta (1, -1) pair, got ${asterGatePair(l192)}`);
  const occ192 = new Set([
    ...l192.cats.map((c) => `${c.x},${c.y}`),
    ...l192.gates.map((g) => `${g.x},${g.y}`),
  ]);
  if (occ192.has("5,5")) throw new Error("L192 Dish must be off Nori seat (5,5)");
  if (occ192.has("0,5") || occ192.has("3,2") || occ192.has("1,4") || occ192.has("5,0")) {
    throw new Error("L192 Dish must be off Urn seats");
  }
  if (occ192.has("3,3") || occ192.has("1,3") || occ192.has("2,4") || occ192.has("3,1")) {
    throw new Error("L192 Dish must be off Vase seats");
  }
  if (occ192.has("1,5") || occ192.has("4,5") || occ192.has("4,4") || occ192.has("3,5")) {
    throw new Error("L192 Dish must be off Planter seats");
  }
  if (occ192.has("4,0") || occ192.has("3,4") || occ192.has("1,0") || occ192.has("4,1")) {
    throw new Error("L192 Dish must be off Tray seats");
  }
  if (occ192.has("2,2") || occ192.has("5,4") || occ192.has("2,3") || occ192.has("0,2")) {
    throw new Error("L192 Dish must be off Sip seats");
  }
  if (occ192.has("2,0") || occ192.has("2,5") || occ192.has("5,3") || occ192.has("1,2")) {
    throw new Error("L192 Dish must be off Saucer seats");
  }
  if (occ192.has("0,3") || occ192.has("4,2") || occ192.has("4,3") || occ192.has("0,4")) {
    throw new Error("L192 Dish must be off L180 Bowl seats");
  }
  if (l192.gates[0].y === l192.gates[1].y) throw new Error("L192 Dish must not same-row Nest");
  if (l192.gates[0].x === l192.gates[1].x) throw new Error("L192 Dish must not column Porch");
  if (l190.cats[0].x === l190.cats[1].x) throw new Error("L190 Petal must not stacked-column Vine");
  if (solves.L190[0][0] === "cat_orange" && solves.L190[0][1] === "n") {
    throw new Error("L190 Petal Cut must not open orange-north (Quill Cut clone)");
  }
  if (solves.L190[0][0] === "cat_orange" && solves.L190[0][1] === "w") {
    throw new Error("L190 Petal Cut must not open orange-west (Spire Cut clone)");
  }
  if (solves.L190[0][0] === "cat_gray" && solves.L190[0][1] === "s") {
    throw new Error("L190 Petal Cut must not open gray-south (Fizz Cut clone)");
  }
  if (solves.L190[0][0] === "cat_orange" && solves.L190[0][1] === "s") {
    throw new Error("L190 Petal Cut must not open orange-south (Vase Stop clone)");
  }
  if (solves.L191[0][0] === "cat_black" && solves.L191[0][1] === "w") {
    throw new Error("L191 Drift Gap must not open black-west (Gleam Gap clone)");
  }
  if (solves.L191[0][0] === "cat_black" && solves.L191[0][1] === "s") {
    throw new Error("L191 Drift Gap must not open black-south (Ember Gap clone)");
  }
  if (solves.L191[0][0] === "cat_orange" && solves.L191[0][1] === "w") {
    throw new Error("L191 Drift Gap must not open orange-west (Flare Gap clone)");
  }
  if (solves.L192[0][0] === "cat_gray" && solves.L192[0][1] === "w") {
    throw new Error("L192 Dish Stop must not open gray-west (Urn Stop clone)");
  }
  if (solves.L192[0][0] === "cat_orange" && solves.L192[0][1] === "n") {
    throw new Error("L192 Dish Stop must not open orange-north (Planter Stop clone)");
  }
  if (solves.L192[0][0] === "cat_orange" && solves.L192[0][1] === "s") {
    throw new Error("L192 Dish Stop must not open orange-south (Vase Stop clone)");
  }
  if (solves.L192[0][0] === "cat_gray" && solves.L192[0][1] === "s") {
    throw new Error("L192 Dish Stop must not open gray-south (Dish Claim clone)");
  }
  for (const level of [l190, l191, l192]) {
  ...
  const wallKeyA = (level: typeof l190) =>
    [...level.walls].map((w) => `${w.x},${w.y}`).sort().join(";");
  const priorWallsA = ["L187", "L188", "L189", "L184", "L185", "L186", "L181", "L182", "L183", "L178", "L179", "L180", "L175", "L176", "L177", "L172", "L173", "L174", "L169", "L170", "L171", "L166", "L167", "L168", "L163", "L164", "L165", "L160", "L161", "L162", "L157", "L158", "L159", "L154", "L155", "L156", "L151", "L152", "L153", "L148", "L149", "L150", "L145", "L146", "L147", "L142", "L143", "L144", "L139", "L140", "L141"].map((id) => wallKeyA(LEVELS.find((r) => r.id === id)!));
  for (const id of ["L190", "L191", "L192"] as const) {
    const key = wallKeyA(LEVELS.find((r) => r.id === id)!);
    if (priorWallsA.includes(key)) throw new Error(`${id} wall twin of L139–L189`);
  }
  if (ART_KIT_PATH.aster.loaf48 !== "/assets/cats/aster_loaf_48.svg") throw new Error("ART_KIT_PATH.aster loaf48");
  if (ART_KIT_PATH.aster.loaf72 !== "/assets/cats/aster_loaf_72.svg") throw new Error("ART_KIT_PATH.aster loaf72");
  ...
  if (!saveA.includes("aster dish")) throw new Error("SaveProvider must yard-bubble aster dish for Aster");
  if (!existsSync(resolve("data/collection/CH4_FRIEND_063.md"))) throw new Error("missing CH4_FRIEND_063.md");
  if (!existsSync(resolve("data/collection/chapter4_aster_bang.json"))) throw new Error("missing collection aster bang");
  if (!existsSync(resolve("data/chapter4_aster_bang.json"))) throw new Error("missing chapter4_aster_bang.json");
  const nameModalA = readFileSync(resolve("src/components/puzzle/NameCatModal.tsx"), "utf8");
  if (!nameModalA.includes("ASTER_FRIEND_ID")) throw new Error("NameCatModal must lockChips Aster");
  if (!nameModalA.includes("ZINNIA_FRIEND_ID")) throw new Error("NameCatModal must keep lockChips Zinnia");
  ...
  console.log("Ch4 Aster@189 + L190–L192 ok · chips Aster/Petal/Drift · coat #7B6BB5 + dusk freckles #2A2040 · Petal Cut / Drift Gap / Dish Stop ·  locked");
}
