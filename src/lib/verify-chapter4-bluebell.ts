...
export function verifyChapter4Bluebell(solves: Record<string, Array<[string, Dir]>>) {
  ...
  if (shippedFriendForClear(222)?.friendId !== "friend_074") throw new Error("onClear(222) must award Bluebell");
  const bluebell = friendById("friend_074");
  if (!bluebell) throw new Error("friend_074 missing from CATALOG");
  if (bluebell.defaultName !== "Bluebell") throw new Error("default name must be Bluebell");
  if (bluebell.unlockClear !== 222) throw new Error("Bluebell unlockClear must be 222");
  if (bluebell.phenotype.artKit !== "bluebell") throw new Error("Bluebell artKit must be bluebell");
  if (bluebell.phenotype.personality !== "Bluebell-soft") throw new Error("Bluebell personality must be Bluebell-soft");
  if (bluebell.phenotype.boardColor !== "orange") throw new Error("Bluebell boardColor must be orange");
  if (bluebell.phenotype.color !== "Woodland") throw new Error("Bluebell color must be Woodland");
  if (bluebell.phenotype.pattern !== "Tipped") throw new Error("Bluebell pattern must be Tipped");
  if (chipsForFriend("friend_074").join(",") !== "Bluebell,Cloche,Ring") {
    throw new Error(`Bluebell chips must be Bluebell/Cloche/Ring, got ${chipsForFriend("friend_074").join(",")}`);
  }
  if (chipsForFriend("friend_074").some((c) => /foxglove|^tower$|^throat$|hyacinth|^cluster$|^bell$|crocus|^saffron$|^tip$|lily|^pollen$|^crest$|violet|^patch$|^moss$|tulip|^stem$|^glow$|poppy|^capsule$|^silk$|lotus|^pad$|^ripple$|orchid|^spur$|^veil$|iris|^blade$|^dew$|aster|^petal$|^drift$|zinnia|^quill$|^gleam$|dahlia|^spire$|^ember$|azalea|^fizz$|^flare$|peony|^bud$|^satin$|camellia|^wax$|^rose$|gardenia|^snow$|^velvet$|hibiscus|^roselle$|^punch$|magnolia|^cream$|^blush$|jasmine|^blossom$|^honey$|bergamot|^citrus$|^earl$|chamomile|^daisy$|^tea$|lavender|^bloom$|^calm$|catnip|^nip$|^dream$|mint|chill|^frost$|^ivory$|^lace$|^sheer$|rosemary|needle|^woody$|thyme|pinch|^twig$|marjoram|softleaf|^peel$|oregano|wild|^bunch$|tarragon|spear|bitters|dill|frondlet|^seed$|parsley|curl|sprig|lovage|^rib$|chervil|frill|lace|fennel|^frond$|anise|pebble|^sage$|^fern$|^clover$|^juniper$/i.test(c))) {
    throw new Error("Bluebell chips must ban  and all prior pools/Pebble/Sage");
  }
  const bluebell48 = readFileSync(resolve("public/assets/cats/bluebell_loaf_48.svg"), "utf8");
  const bluebell72 = readFileSync(resolve("public/assets/cats/bluebell_loaf_72.svg"), "utf8");
  if (!bluebell48.includes("#4A7EC8") || !bluebell72.includes("#4A7EC8")) throw new Error("Bluebell loaf must use woodland-bluebell coat #4A7EC8");
  if (!bluebell48.includes("#1A2848") || !bluebell72.includes("#1A2848")) throw new Error("Bluebell loaf must use tip freckles #1A2848");
  if (!bluebell48.includes("#D0E4F6") || !bluebell72.includes("#D0E4F6")) throw new Error("Bluebell loaf must use belly #D0E4F6");
  for (const hex of ["#D478A0", "#3A2030", "#B44A78", "#F8D4E6", "#FFF4FA", "#5A6EC8", "#1E2448", "#C8D2F0", "#3A4A88", "#F0F4FF", "#C45A9A", "#2A1830", "#F4C2DC", "#9A3A78", "#FFF0F8", "#F5F0E6", "#C8A84A", "#FFF8EC", "#C4B06A", "#FEF6DC", "#6B4AA0", "#1E2A18", "#D2C4EA", "#4A3078", "#F8F4FF", "#E07090", "#2A4020", "#F8C4D2", "#C44868", "#D94A5A", "#3A1218", "#F4B4B8", "#B02A3A", "#F2B8C8", "#4A2840", "#FAD6E0", "#D4899A", "#FFF5F8", "#C989B8", "#3A2038", "#EAC8DC", "#A86A96", "#FFE6F4", "#5B4F9A", "#1C1630", "#B6A8DE", "#3D3474", "#E4DCFF", "#7B6BB5", "#2A2040", "#C8B8EE", "#5A4A92", "#EDE6FF", "#E85A2A", "#3A1808", "#F4A06A", "#B33A14", "#8B2E4A", "#2A1018", "#C86A80", "#5C1832", "#B84A8C", "#3F1830", "#E8A0C4", "#7A2458", "#E8B4C8", "#5A3048", "#F7DCE6", "#C4789A", "#C45A6A", "#5A2030", "#F3C8CC", "#8E3848", "#F6F1E8", "#4F6B4A", "#FFFBF3", "#C5C0B2", "#D46A8A", "#5A1F3A", "#C24A6E", "#EBB0C2", "#F7E8D2", "#C48A7A", "#FFF4E4", "#E8B8A4", "#F2D4C4", "#6E3A42", "#F4EFE6", "#7A6B4E", "#FBF7EE", "#F0C98A", "#9A5A1A", "#FFF1C4", "#E8D5A3", "#8A6B2E", "#F7E8C4", "#C4A66A", "#B8A0C8", "#5A3F6E", "#EDE4F4", "#8A6AA8", "#8FBF9A", "#2F5C3A", "#DCEFE2", "#7EC8A3", "#2F6B52", "#D4F4E8", "#7A9B88", "#2F463C", "#C8D6CE", "#A3B57C", "#4E5C36", "#D2D8B0", "#8FA86A", "#3F5230", "#D8D4A8", "#7A9A4E", "#4A5C2E", "#6B8F4E", "#2F4A28", "#C4D6A4", "#8FBF7A", "#4A6B3E", "#6FA86A", "#3F6B3C", "#9CB87A", "#5E7348", "#C6D9B4", "#5E7F52", "#E6C86E", "#7A9B6A", "#6A7D6E", "#1A2C24", "#7E8F86", "#C8D24A", "#EBE3C4", "#C9C09A", "#F3EBD0"]) {
    if (bluebell48.includes(hex) || bluebell72.includes(hex)) throw new Error(`Bluebell loaf must not use prior herb coat ${hex}`);
  }
  if (furnitureGiftsForClear(222).length) throw new Error("Bluebell@222 must gift no furniture");
  const yardF = readFileSync(resolve("src/components/yard/YardScene.tsx"), "utf8");
  if ((yardF.match(/const bluebell =/g) || []).length !== 1) throw new Error("YardScene must declare bluebell once");
  if ((yardF.match(/\{bluebell \?/g) || []).length !== 1) throw new Error("YardScene must render bluebell once");
  ...
  if (priorOcc.has(occ222)) throw new Error("L225 Chime must not twin Cup/Pad/Pond/Vase/Bloom/Thicket/Anther/Stigma/Raceme/Glove occupancy");
  if (occ222.split(";").includes("5,5")) throw new Error("L225 Chime must be off Nori seat (5,5)");
  if (l225.gates[0].y === l225.gates[1].y) throw new Error("L225 Chime must not same-row Nest");
  if (l225.gates[0].x === l225.gates[1].x) throw new Error("L225 Chime must not column Porch");
  if (l223.cats[0].x === l223.cats[1].x) throw new Error("L223 Cloche must not stacked-column Vine");
  if (solves.L223[0][0] === "cat_orange" && solves.L223[0][1] === "s") {
    throw new Error("L223 Cloche Cut must not open orange-south (Tower Cut clone)");
  }
  if (solves.L223[0][0] === "cat_black" && solves.L223[0][1] === "w") {
    throw new Error("L223 Cloche Cut must not open black-west (Pollen Cut clone)");
  }
  if (solves.L223[0][0] === "cat_gray" && solves.L223[0][1] === "w") {
    throw new Error("L223 Cloche Cut must not open gray-west (Pad Cut clone)");
  }
  if (solves.L223[0][0] === "cat_orange" && solves.L223[0][1] === "e") {
    throw new Error("L223 Cloche Cut must not open orange-east (Spur Cut clone)");
  }
  if (solves.L223[0][0] === "cat_gray" && solves.L223[0][1] === "n") {
    throw new Error("L223 Cloche Cut must not open gray-north (Blade Cut clone)");
  }
  if (solves.L223[0][0] === "cat_gray" && solves.L223[0][1] === "e") {
    throw new Error("L223 Cloche Cut must not open gray-east (Petal Cut clone)");
  }
  if (solves.L223[0][0] === "cat_orange" && solves.L223[0][1] === "n") {
    throw new Error("L223 Cloche Cut must not open orange-north (Quill Cut clone)");
  }
  if (solves.L223[0][0] === "cat_black" && solves.L223[0][1] === "n") {
    throw new Error("L223 Cloche Cut must not open black-north (Cluster Cut clone)");
  }
  if (solves.L223[0][0] === "cat_black" && solves.L223[0][1] === "s") {
    throw new Error("L223 Cloche Cut must not open black-south (Stem Cut clone)");
  }
  if (solves.L223[0][0] === "cat_black" && solves.L223[0][1] === "e") {
    throw new Error("L223 Cloche Cut must not open black-east (Patch Cut clone)");
  }
  if (solves.L223[0][0] === "cat_orange" && solves.L223[0][1] === "w") {
    throw new Error("L223 Cloche Cut must not open orange-west (Saffron Cut clone)");
  }
  if (solves.L224[0][0] === "cat_gray" && solves.L224[0][1] === "w") {
    throw new Error("L224 Ring Gap must not open gray-west (Throat Gap clone)");
  }
  if (solves.L224[0][0] === "cat_gray" && solves.L224[0][1] === "s") {
    throw new Error("L224 Ring Gap must not open gray-south (Bell Gap clone)");
  }
  if (solves.L224[0][0] === "cat_black" && solves.L224[0][1] === "e") {
    throw new Error("L224 Ring Gap must not open black-east (Ripple Gap clone)");
  }
  if (solves.L224[0][0] === "cat_orange" && solves.L224[0][1] === "s") {
    throw new Error("L224 Ring Gap must not open orange-south (Veil Gap clone)");
  }
  if (solves.L224[0][0] === "cat_orange" && solves.L224[0][1] === "n") {
    throw new Error("L224 Ring Gap must not open orange-north (Dew Gap clone)");
  }
  if (solves.L224[0][0] === "cat_black" && solves.L224[0][1] === "w") {
    throw new Error("L224 Ring Gap must not open black-west (Gleam Gap clone)");
  }
  if (solves.L224[0][0] === "cat_black" && solves.L224[0][1] === "s") {
    throw new Error("L224 Ring Gap must not open black-south (Ember Gap clone)");
  }
  if (solves.L224[0][0] === "cat_gray" && solves.L224[0][1] === "e") {
    throw new Error("L224 Ring Gap must not open gray-east (Glow Gap clone)");
  }
  if (solves.L224[0][0] === "cat_gray" && solves.L224[0][1] === "n") {
    throw new Error("L224 Ring Gap must not open gray-north (Moss Gap clone)");
  }
  if (solves.L224[0][0] === "cat_orange" && solves.L224[0][1] === "e") {
    throw new Error("L224 Ring Gap must not open orange-east (Tip Gap clone)");
  }
  if (solves.L225[0][0] === "cat_black" && solves.L225[0][1] === "w") {
    throw new Error("L225 Chime Stop must not open black-west (Glove Stop clone)");
  }
  if (solves.L225[0][0] === "cat_black" && solves.L225[0][1] === "e") {
    throw new Error("L225 Chime Stop must not open black-east (Raceme Stop clone)");
  }
  if (solves.L225[0][0] === "cat_orange" && solves.L225[0][1] === "w") {
    throw new Error("L225 Chime Stop must not open orange-west (Spike Stop clone)");
  }
  if (solves.L225[0][0] === "cat_gray" && solves.L225[0][1] === "n") {
    throw new Error("L225 Chime Stop must not open gray-north (Dish Stop clone)");
  }
  if (solves.L225[0][0] === "cat_gray" && solves.L225[0][1] === "w") {
    throw new Error("L225 Chime Stop must not open gray-west (Urn Stop clone)");
  }
  if (solves.L225[0][0] === "cat_orange" && solves.L225[0][1] === "s") {
    throw new Error("L225 Chime Stop must not open orange-south (Vase Stop clone)");
  }
  if (solves.L225[0][0] === "cat_gray" && solves.L225[0][1] === "s") {
    throw new Error("L225 Chime Stop must not open gray-south (Dish Claim clone)");
  }
  if (solves.L225[0][0] === "cat_gray" && solves.L225[0][1] === "e") {
    throw new Error("L225 Chime Stop must not open gray-east (Petal Cut clone)");
  }
  if (solves.L225[0][0] === "cat_black" && solves.L225[0][1] === "n") {
    throw new Error("L225 Chime Stop must not open black-north (Bloom Stop clone)");
  }
  if (solves.L225[0][0] === "cat_black" && solves.L225[0][1] === "s") {
    throw new Error("L225 Chime Stop must not open black-south (Thicket Stop clone)");
  }
  if (solves.L225[0][0] === "cat_orange" && solves.L225[0][1] === "e") {
    throw new Error("L225 Chime Stop must not open orange-east (Stigma Stop clone)");
  }
  for (const level of [l223, l224, l225]) {
  ...
  const wallKeyO = (level: typeof l223) =>
    [...level.walls].map((w) => `${w.x},${w.y}`).sort().join(";");
  const priorWallsO = ["L220", "L221", "L222", "L217", "L218", "L219", "L214", "L215", "L216", "L211", "L212", "L213", "L208", "L209", "L210", "L205", "L206", "L207", "L202", "L203", "L204", "L199", "L200", "L201", "L196", "L197", "L198", "L193", "L194", "L195", "L190", "L191", "L192", "L187", "L188", "L189", "L184", "L185", "L186", "L181", "L182", "L183", "L178", "L179", "L180", "L175", "L176", "L177", "L172", "L173", "L174", "L169", "L170", "L171", "L166", "L167", "L168", "L163", "L164", "L165", "L160", "L161", "L162", "L157", "L158", "L159", "L154", "L155", "L156", "L151", "L152", "L153", "L148", "L149", "L150", "L145", "L146", "L147", "L142", "L143", "L144", "L139", "L140", "L141"].map((id) => wallKeyO(LEVELS.find((r) => r.id === id)!));
  for (const id of ["L223", "L224", "L225"] as const) {
    const key = wallKeyO(LEVELS.find((r) => r.id === id)!);
    if (priorWallsO.includes(key)) throw new Error(`${id} wall twin of L139–L222`);
  }
  if (ART_KIT_PATH.bluebell.loaf48 !== "/assets/cats/bluebell_loaf_48.svg") throw new Error("ART_KIT_PATH.bluebell loaf48");
  if (ART_KIT_PATH.bluebell.loaf72 !== "/assets/cats/bluebell_loaf_72.svg") throw new Error("ART_KIT_PATH.bluebell loaf72");
  ...
  if (!saveF.includes("bluebell cloche")) throw new Error("SaveProvider must yard-bubble bluebell cloche for Bluebell");
  ...
  if (!existsSync(resolve("data/collection/CH4_FRIEND_074.md"))) throw new Error("missing CH4_FRIEND_074.md");
  if (!existsSync(resolve("data/collection/chapter4_bluebell_bang.json"))) throw new Error("missing collection bluebell bang");
  if (!existsSync(resolve("data/chapter4_bluebell_bang.json"))) throw new Error("missing chapter4_bluebell_bang.json");
  const nameModalF = readFileSync(resolve("src/components/puzzle/NameCatModal.tsx"), "utf8");
  if (!nameModalF.includes("BLUEBELL_FRIEND_ID")) throw new Error("NameCatModal must lockChips Bluebell");
  ...
  console.log("Ch4 Bluebell@222 + L223–L225 ok · chips Bluebell/Cloche/Ring · coat #4A7EC8 + tip freckles #1A2848 · Cloche Cut / Ring Gap / Chime Stop ·  locked");
}
