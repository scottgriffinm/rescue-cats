...
export function verifyChapter4Iris(solves: Record<string, Array<[string, Dir]>>) {
  ...
  if (SLICE_UNLOCKS[192] !== "friend_064") throw new Error("SLICE_UNLOCKS[192] must be friend_064");
  if (shippedFriendForClear(192)?.friendId !== "friend_064") throw new Error("onClear(192) must award Iris");
  const iris = friendById("friend_064");
  if (!iris) throw new Error("friend_064 missing from CATALOG");
  if (iris.defaultName !== "Iris") throw new Error("default name must be Iris");
  if (iris.unlockClear !== 192) throw new Error("Iris unlockClear must be 192");
  if (iris.phenotype.artKit !== "iris") throw new Error("Iris artKit must be iris");
  if (iris.phenotype.personality !== "Iris-soft") throw new Error("Iris personality must be Iris-soft");
  if (iris.phenotype.boardColor !== "orange") throw new Error("Iris boardColor must be orange");
  if (iris.phenotype.color !== "Orange") throw new Error("Iris color must be Orange");
  if (iris.phenotype.pattern !== "Freckled") throw new Error("Iris pattern must be Freckled");
  if (chipsForFriend("friend_064").join(",") !== "Iris,Blade,Dew") {
    throw new Error(`Iris chips must be Iris/Blade/Dew, got ${chipsForFriend("friend_064").join(",")}`);
  }
  if (chipsForFriend("friend_064").some((c) => /aster|^petal$|^drift$|zinnia|^quill$|^gleam$|dahlia|^spire$|^ember$|azalea|^fizz$|^flare$|peony|^bud$|^satin$|camellia|^wax$|^rose$|gardenia|^snow$|^velvet$|hibiscus|^roselle$|^punch$|magnolia|^cream$|^blush$|jasmine|^blossom$|^honey$|bergamot|^citrus$|^earl$|chamomile|^daisy$|^tea$|lavender|^bloom$|^calm$|catnip|^nip$|^dream$|mint|chill|^frost$|^ivory$|^lace$|^sheer$|rosemary|needle|^woody$|thyme|pinch|^twig$|marjoram|softleaf|^peel$|oregano|wild|^bunch$|tarragon|spear|bitters|dill|frondlet|^seed$|parsley|curl|sprig|lovage|stem|rib|chervil|frill|lace|fennel|^frond$|anise|pebble|^sage$/i.test(c))) {
    throw new Error("Iris chips must ban Aster/Petal/Drift/Zinnia/Quill/Gleam/Dahlia/Spire/Ember/Azalea/Fizz/Flare/Peony/Bud/Satin/Camellia/Wax/Rose/Gardenia/Snow/Velvet/Hibiscus/Roselle/Punch/Magnolia/Cream/Blush/Jasmine/Blossom/Honey and all prior pools/Pebble/Sage");
  }
  const iris48 = readFileSync(resolve("public/assets/cats/iris_loaf_48.svg"), "utf8");
  const iris72 = readFileSync(resolve("public/assets/cats/iris_loaf_72.svg"), "utf8");
  if (!iris48.includes("#5B4F9A") || !iris72.includes("#5B4F9A")) throw new Error("Iris loaf must use deep indigo-violet coat #5B4F9A");
  if (!iris48.includes("#1C1630") || !iris72.includes("#1C1630")) throw new Error("Iris loaf must use dusk freckles #1C1630");
  if (!iris48.includes("#B6A8DE") || !iris72.includes("#B6A8DE")) throw new Error("Iris loaf must use belly #B6A8DE");
  for (const hex of ["#7B6BB5", "#2A2040", "#C8B8EE", "#5A4A92", "#EDE6FF", "#E85A2A", "#3A1808", "#F4A06A", "#B33A14", "#8B2E4A", "#2A1018", "#C86A80", "#5C1832", "#B84A8C", "#3F1830", "#E8A0C4", "#7A2458", "#E8B4C8", "#5A3048", "#F7DCE6", "#C4789A", "#C45A6A", "#5A2030", "#F3C8CC", "#8E3848", "#F6F1E8", "#4F6B4A", "#FFFBF3", "#C5C0B2", "#D46A8A", "#5A1F3A", "#C24A6E", "#EBB0C2", "#F7E8D2", "#C48A7A", "#FFF4E4", "#E8B8A4", "#F2D4C4", "#6E3A42", "#F4EFE6", "#7A6B4E", "#FBF7EE", "#F0C98A", "#9A5A1A", "#FFF1C4", "#E8D5A3", "#8A6B2E", "#F7E8C4", "#C4A66A", "#B8A0C8", "#5A3F6E", "#EDE4F4", "#8A6AA8", "#8FBF9A", "#2F5C3A", "#DCEFE2", "#7EC8A3", "#2F6B52", "#D4F4E8", "#7A9B88", "#2F463C", "#C8D6CE", "#A3B57C", "#4E5C36", "#D2D8B0", "#8FA86A", "#3F5230", "#D8D4A8", "#7A9A4E", "#4A5C2E", "#6B8F4E", "#2F4A28", "#C4D6A4", "#8FBF7A", "#4A6B3E", "#6FA86A", "#3F6B3C", "#9CB87A", "#5E7348", "#C6D9B4", "#5E7F52", "#E6C86E", "#7A9B6A", "#6A7D6E", "#1A2C24", "#7E8F86", "#C8D24A", "#EBE3C4", "#C9C09A", "#F3EBD0"]) {
    if (iris48.includes(hex) || iris72.includes(hex)) throw new Error(`Iris loaf must not use prior herb coat ${hex}`);
  }
  if (furnitureGiftsForClear(192).length) throw new Error("Iris@192 must gift no furniture");
  ...
  if (TUTORIAL_RESCUES.length !== 97) throw new Error("Met must include through Iris (64)");
  ...
  const l193 = LEVELS.find((r) => r.id === "L193")!;
  const l194 = LEVELS.find((r) => r.id === "L194")!;
  const l195 = LEVELS.find((r) => r.id === "L195")!;
  if (/\b(petal|drift|dish|aster|quill|gleam|urn|zinnia|spire|ember|vase|dahlia|fizz|flare|planter|azalea|wax|rose|tray|snow|velvet|latch|claim|roselle|punch|sip|hibiscus|cream|blush|magnolia|blossom|honey|bloom|citrus|earl|saucer|daisy|tea|cup|calm|bundle|nip|dream|pouch|chill|frost|tin|needle|woody|pot|pinch|twig|jar|softleaf|dusty|peel|stone|wild|pizza|spear|bitters|cruet|seed|frondlet|ledge|garnish|bed|gardenia|camellia|peony|bud|satin|bowl)\b/i.test([l193.name, l194.name].join(","))) {
    throw new Error("L193–L194 must not reuse Petal/Drift/Dish/Aster/Quill/Gleam/Urn/Zinnia/Spire/Ember/Vase/Dahlia/Fizz/Flare/Planter/Azalea/Wax/Rose/Tray/Snow/Velvet/Roselle/Punch/Sip/Cream/Blush/Cup/Bloom/Saucer/Peony/Bud/Satin/Bowl and prior triad names");
  }
  const irisGatePair = (level: typeof l193) => level.gates.map((g) => `${g.x},${g.y}`).sort().join("/");
  if (irisGatePair(l193) !== "1,2/4,0") throw new Error(`L193 gates must be delta (-3, 2) pair, got ${irisGatePair(l193)}`);
  if (irisGatePair(l194) !== "1,1/5,3") throw new Error(`L194 gates must be delta (-4, -2) pair, got ${irisGatePair(l194)}`);
  if (irisGatePair(l195) !== "1,0/4,2") throw new Error(`L195 gates must be delta (3, 2) pair, got ${irisGatePair(l195)}`);
  const occ195 = new Set([
    ...l195.cats.map((c) => `${c.x},${c.y}`),
    ...l195.gates.map((g) => `${g.x},${g.y}`),
  ]);
  if (occ195.has("5,5")) throw new Error("L195 Stem must be off Nori seat (5,5)");
  if (occ195.has("5,2") || occ195.has("5,1") || occ195.has("3,0") || occ195.has("2,1")) {
    throw new Error("L195 Stem must be off Dish seats");
  }
  if (occ195.has("0,5") || occ195.has("3,2") || occ195.has("1,4") || occ195.has("5,0")) {
    throw new Error("L195 Stem must be off Urn seats");
  }
  if (occ195.has("3,3") || occ195.has("1,3") || occ195.has("2,4") || occ195.has("3,1")) {
    throw new Error("L195 Stem must be off Vase seats");
  }
  if (occ195.has("1,5") || occ195.has("4,5") || occ195.has("4,4") || occ195.has("3,5")) {
    throw new Error("L195 Stem must be off Planter seats");
  }
  if (occ195.has("2,2") || occ195.has("5,4") || occ195.has("2,3") || occ195.has("0,2")) {
    throw new Error("L195 Stem must be off Sip seats");
  }
  if (occ195.has("2,0") || occ195.has("2,5") || occ195.has("5,3") || occ195.has("1,2")) {
    throw new Error("L195 Stem must be off Saucer seats");
  }
  if (l195.gates[0].y === l195.gates[1].y) throw new Error("L195 Stem must not same-row Nest");
  if (l195.gates[0].x === l195.gates[1].x) throw new Error("L195 Stem must not column Porch");
  if (l193.cats[0].x === l193.cats[1].x) throw new Error("L193 Blade must not stacked-column Vine");
  if (solves.L193[0][0] === "cat_gray" && solves.L193[0][1] === "e") {
    throw new Error("L193 Blade Cut must not open gray-east (Petal Cut clone)");
  }
  if (solves.L193[0][0] === "cat_orange" && solves.L193[0][1] === "n") {
    throw new Error("L193 Blade Cut must not open orange-north (Quill Cut clone)");
  }
  if (solves.L193[0][0] === "cat_orange" && solves.L193[0][1] === "w") {
    throw new Error("L193 Blade Cut must not open orange-west (Spire Cut clone)");
  }
  if (solves.L193[0][0] === "cat_gray" && solves.L193[0][1] === "s") {
    throw new Error("L193 Blade Cut must not open gray-south (Fizz Cut clone)");
  }
  if (solves.L193[0][0] === "cat_orange" && solves.L193[0][1] === "s") {
    throw new Error("L193 Blade Cut must not open orange-south (Vase Stop clone)");
  }
  if (solves.L194[0][0] === "cat_orange" && solves.L194[0][1] === "e") {
    throw new Error("L194 Dew Gap must not open orange-east (Drift Gap clone)");
  }
  if (solves.L194[0][0] === "cat_black" && solves.L194[0][1] === "w") {
    throw new Error("L194 Dew Gap must not open black-west (Gleam Gap clone)");
  }
  if (solves.L194[0][0] === "cat_black" && solves.L194[0][1] === "s") {
    throw new Error("L194 Dew Gap must not open black-south (Ember Gap clone)");
  }
  if (solves.L194[0][0] === "cat_orange" && solves.L194[0][1] === "w") {
    throw new Error("L194 Dew Gap must not open orange-west (Flare Gap clone)");
  }
  if (solves.L195[0][0] === "cat_gray" && solves.L195[0][1] === "n") {
    throw new Error("L195 Stem Stop must not open gray-north (Dish Stop clone)");
  }
  if (solves.L195[0][0] === "cat_gray" && solves.L195[0][1] === "w") {
    throw new Error("L195 Stem Stop must not open gray-west (Urn Stop clone)");
  }
  if (solves.L195[0][0] === "cat_orange" && solves.L195[0][1] === "n") {
    throw new Error("L195 Stem Stop must not open orange-north (Planter Stop clone)");
  }
  if (solves.L195[0][0] === "cat_orange" && solves.L195[0][1] === "s") {
    throw new Error("L195 Stem Stop must not open orange-south (Vase Stop clone)");
  }
  if (solves.L195[0][0] === "cat_gray" && solves.L195[0][1] === "s") {
    throw new Error("L195 Stem Stop must not open gray-south (Dish Claim clone)");
  }
  if (solves.L195[0][0] === "cat_gray" && solves.L195[0][1] === "e") {
    throw new Error("L195 Stem Stop must not open gray-east (Petal Cut clone)");
  }
  for (const level of [l193, l194, l195]) {
  ...
  const wallKeyI = (level: typeof l193) =>
    [...level.walls].map((w) => `${w.x},${w.y}`).sort().join(";");
  const priorWallsI = ["L190", "L191", "L192", "L187", "L188", "L189", "L184", "L185", "L186", "L181", "L182", "L183", "L178", "L179", "L180", "L175", "L176", "L177", "L172", "L173", "L174", "L169", "L170", "L171", "L166", "L167", "L168", "L163", "L164", "L165", "L160", "L161", "L162", "L157", "L158", "L159", "L154", "L155", "L156", "L151", "L152", "L153", "L148", "L149", "L150", "L145", "L146", "L147", "L142", "L143", "L144", "L139", "L140", "L141"].map((id) => wallKeyI(LEVELS.find((r) => r.id === id)!));
  for (const id of ["L193", "L194", "L195"] as const) {
    const key = wallKeyI(LEVELS.find((r) => r.id === id)!);
    if (priorWallsI.includes(key)) throw new Error(`${id} wall twin of L139–L192`);
  }
  if (ART_KIT_PATH.iris.loaf48 !== "/assets/cats/iris_loaf_48.svg") throw new Error("ART_KIT_PATH.iris loaf48");
  if (ART_KIT_PATH.iris.loaf72 !== "/assets/cats/iris_loaf_72.svg") throw new Error("ART_KIT_PATH.iris loaf72");
  ...
  if (!saveI.includes("iris stem")) throw new Error("SaveProvider must yard-bubble iris stem for Iris");
  if (!existsSync(resolve("data/collection/CH4_FRIEND_064.md"))) throw new Error("missing CH4_FRIEND_064.md");
  if (!existsSync(resolve("data/collection/chapter4_iris_bang.json"))) throw new Error("missing collection iris bang");
  if (!existsSync(resolve("data/chapter4_iris_bang.json"))) throw new Error("missing chapter4_iris_bang.json");
  const nameModalI = readFileSync(resolve("src/components/puzzle/NameCatModal.tsx"), "utf8");
  if (!nameModalI.includes("IRIS_FRIEND_ID")) throw new Error("NameCatModal must lockChips Iris");
  if (!nameModalI.includes("ASTER_FRIEND_ID")) throw new Error("NameCatModal must keep lockChips Aster");
  if (!nameModalI.includes("ZINNIA_FRIEND_ID")) throw new Error("NameCatModal must keep lockChips Zinnia");
  ...
  console.log("Ch4 Iris@192 + L193–L195 ok · chips Iris/Blade/Dew · coat #5B4F9A + dusk freckles #1C1630 · Blade Cut / Dew Gap / Stem Stop ·  locked");
}
