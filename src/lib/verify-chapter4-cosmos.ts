...
export function verifyChapter4Cosmos(solves: Record<string, Array<[string, Dir]>>) {
  for (const [id, name] of [
    ...
    if (/cosmos (cut|gap|stop)/i.test(level.name)) {
      throw new Error(`${id} must not be Cosmos Cut/Gap/Stop`);
    }
  ...
  if (SLICE_UNLOCKS[240] !== "friend_080") throw new Error("SLICE_UNLOCKS[240] must be friend_080");
  if (shippedFriendForClear(240)?.friendId !== "friend_080") throw new Error("onClear(240) must award Cosmos");
  const cosmos = friendById("friend_080");
  if (!cosmos) throw new Error("friend_080 missing from CATALOG");
  if (cosmos.defaultName !== "Cosmos") throw new Error("default name must be Cosmos");
  if (cosmos.unlockClear !== 240) throw new Error("Cosmos unlockClear must be 240");
  if (cosmos.phenotype.artKit !== "cosmos") throw new Error("Cosmos artKit must be cosmos");
  if (cosmos.phenotype.personality !== "Cosmos-soft") throw new Error("Cosmos personality must be Cosmos-soft");
  if (cosmos.phenotype.boardColor !== "orange") throw new Error("Cosmos boardColor must be orange");
  if (cosmos.phenotype.color !== "Airy") throw new Error("Cosmos color must be Airy");
  if (cosmos.phenotype.pattern !== "Ray") throw new Error("Cosmos pattern must be Ray");
  if (chipsForFriend("friend_080").join(",") !== "Cosmos,Airy,Ray") {
    throw new Error(`Cosmos chips must be Cosmos/Airy/Ray, got ${chipsForFriend("friend_080").join(",")}`);
  }
  if (chipsForFriend("friend_080").some((c) => /buttercup|^meadow$|^gloss$|primrose|^pale$|^dish$|heather|^moor$|^sprig$|marigold|^gold$|^pot$|snapdragon|^jaw$|^perch$|bluebell|^cloche$|^ring$|foxglove|^tower$|^throat$|hyacinth|^cluster$|^bell$|crocus|^saffron$|^tip$|lily|^pollen$|^crest$|violet|^patch$|^moss$|tulip|^stem$|^glow$|poppy|^capsule$|^silk$|lotus|^pad$|^ripple$|orchid|^spur$|^veil$|iris|^blade$|^dew$|aster|^petal$|^drift$|zinnia|^quill$|^gleam$|dahlia|^spire$|^ember$|azalea|^fizz$|^flare$|peony|^bud$|^satin$|camellia|^wax$|^rose$|gardenia|^snow$|^velvet$|hibiscus|^roselle$|^punch$|magnolia|^cream$|^blush$|jasmine|^blossom$|^honey$|bergamot|^citrus$|^earl$|chamomile|^daisy$|^tea$|lavender|^bloom$|^calm$|catnip|^nip$|^dream$|mint|chill|^frost$|^ivory$|^lace$|^sheer$|rosemary|needle|^woody$|thyme|pinch|^twig$|marjoram|softleaf|^peel$|oregano|wild|^bunch$|tarragon|spear|bitters|dill|frondlet|^seed$|parsley|curl|lovage|^rib$|chervil|frill|lace|fennel|^frond$|anise|pebble|^sage$|^fern$|^clover$|^juniper$/i.test(c))) {
    throw new Error("Cosmos chips must ban  and all prior pools/Pebble/Sage");
  }
  const cosmos48 = readFileSync(resolve("public/assets/cats/cosmos_loaf_48.svg"), "utf8");
  const cosmos72 = readFileSync(resolve("public/assets/cats/cosmos_loaf_72.svg"), "utf8");
  if (!cosmos48.includes("#E8A0C0") || !cosmos72.includes("#E8A0C0")) throw new Error("Cosmos loaf must use airy cosmos coat #E8A0C0");
  if (!cosmos48.includes("#5A2848") || !cosmos72.includes("#5A2848")) throw new Error("Cosmos loaf must use center freckles #5A2848");
  if (!cosmos48.includes("#FFF9FD") || !cosmos72.includes("#FFF9FD")) throw new Error("Cosmos loaf must use belly #FFF9FD");
  for (const hex of ["#F5D030", "#6A4A10", "#FFFCE8", "#E8B810", "#F2D4A0", "#8A5A20", "#C89648", "#FFF8E0", "#9A5A8A", "#2A1830", "#7A3F6C", "#F0D6E8", "#FFF2F8", "#F0A020", "#4A3010", "#FFE9B4", "#C86E0C", "#FFF6D0", "#E07050", "#3A2010", "#B84A28", "#F3C8A8", "#C8A84A", "#C4B06A", "#4A7EC8", "#1A2848", "#D0E4F6", "#2E5A98", "#E8F4FF", "#D478A0", "#3A2030", "#B44A78", "#F8D4E6", "#FFF4FA", "#5A6EC8", "#1E2448", "#C8D2F0", "#3A4A88", "#F0F4FF", "#C45A9A", "#F4C2DC", "#9A3A78", "#FFF0F8", "#F5F0E6", "#C8A84A", "#FFF8EC", "#C4B06A", "#FEF6DC", "#6B4AA0", "#1E2A18", "#D2C4EA", "#4A3078", "#F8F4FF", "#E07090", "#2A4020", "#F8C4D2", "#C44868", "#D94A5A", "#3A1218", "#F4B4B8", "#B02A3A", "#F2B8C8", "#4A2840", "#FAD6E0", "#D4899A", "#FFF5F8", "#C989B8", "#3A2038", "#EAC8DC", "#A86A96", "#FFE6F4", "#5B4F9A", "#1C1630", "#B6A8DE", "#3D3474", "#E4DCFF", "#7B6BB5", "#2A2040", "#C8B8EE", "#5A4A92", "#EDE6FF", "#E85A2A", "#3A1808", "#F4A06A", "#B33A14", "#8B2E4A", "#2A1018", "#C86A80", "#5C1832", "#B84A8C", "#3F1830", "#E8A0C4", "#7A2458", "#E8B4C8", "#5A3048", "#F7DCE6", "#C4789A", "#C45A6A", "#5A2030", "#F3C8CC", "#8E3848", "#F6F1E8", "#4F6B4A", "#FFFBF3", "#C5C0B2", "#D46A8A", "#5A1F3A", "#C24A6E", "#EBB0C2", "#F7E8D2", "#C48A7A", "#FFF4E4", "#E8B8A4", "#F2D4C4", "#6E3A42", "#F4EFE6", "#7A6B4E", "#FBF7EE", "#F0C98A", "#9A5A1A", "#FFF1C4", "#E8D5A3", "#8A6B2E", "#F7E8C4", "#C4A66A", "#B8A0C8", "#5A3F6E", "#EDE4F4", "#8A6AA8", "#8FBF9A", "#2F5C3A", "#DCEFE2", "#7EC8A3", "#2F6B52", "#D4F4E8", "#7A9B88", "#2F463C", "#C8D6CE", "#A3B57C", "#4E5C36", "#D2D8B0", "#8FA86A", "#3F5230", "#D8D4A8", "#7A9A4E", "#4A5C2E", "#6B8F4E", "#2F4A28", "#C4D6A4", "#8FBF7A", "#4A6B3E", "#6FA86A", "#3F6B3C", "#9CB87A", "#5E7348", "#C6D9B4", "#5E7F52", "#E6C86E", "#7A9B6A", "#6A7D6E", "#1A2C24", "#7E8F86", "#C8D24A", "#EBE3C4", "#C9C09A", "#F3EBD0"]) {
    if (cosmos48.includes(hex) || cosmos72.includes(hex)) throw new Error(`Cosmos loaf must not use prior herb coat ${hex}`);
  }
  if (furnitureGiftsForClear(240).length) throw new Error("Cosmos@240 must gift no furniture");
  const yardF = readFileSync(resolve("src/components/yard/YardScene.tsx"), "utf8");
  if ((yardF.match(/const cosmos =/g) || []).length !== 1) throw new Error("YardScene must declare cosmos once");
  if ((yardF.match(/\{cosmos \?/g) || []).length !== 1) throw new Error("YardScene must render cosmos once");
  ...
  if (TUTORIAL_RESCUES.length !== 82) throw new Error("Met must include through Cosmos (80) after Wisteria (82)");
  ...
  if (SLICE_UNLOCKS[240] !== "friend_080") throw new Error("SLICE_UNLOCKS[240] must be friend_080");
  if (SLICE_UNLOCKS[243] !== "friend_081") throw new Error("SLICE_UNLOCKS[243] must be friend_081 after Clematis ship");
  if (SLICE_UNLOCKS[246] !== "friend_082") throw new Error("SLICE_UNLOCKS[246] must be friend_082 after Wisteria ship");
  if (/pebble|Pebble/i.test(cosmos48 + cosmos72)) throw new Error("Cosmos art must not use Pebble");
  if (/buttercup|meadow|gloss|petal|primrose|pale|dish|ruff|heather|moor|sprig|heath|marigold|gold|pot|snapdragon|jaw|perch|bluebell|cloche|ring|foxglove|tower|throat|hyacinth|cluster|raceme|crocus|saffron|stigma|lily|pollen|crest|anther|violet|patch|moss|thicket|tulip|stem|glow|poppy|capsule|silk|lotus|pad|ripple|orchid|spur|veil|iris|blade|dew|aster|petal|drift|zinnia|quill|gleam|dahlia|spire|ember|azalea|fizz|flare|peony|camellia|wax|gardenia|hibiscus|roselle|punch|magnolia|cream|blush|jasmine|blossom|honey|bergamot|citrus|earl|chamomile|daisy|tea|lavender|bloom|calm|catnip|mint|chill|frost|ivory|sheer|rosemary|needle|woody|thyme|pinch|twig|marjoram|softleaf|peel|oregano|wild|bunch|tarragon|spear|bitters|dill|parsley|curl|lovage|rib|chervil|frill|lace|fennel|anise/i.test(cosmos48 + cosmos72)) {
    throw new Error("Cosmos art must not collide  marks");
  }
  const l241 = LEVELS.find((r) => r.id === "L241")!;
  const l242 = LEVELS.find((r) => r.id === "L242")!;
  const l243 = LEVELS.find((r) => r.id === "L243")!;
  if (/\b(meadow|gloss|petal|buttercup|pale|dish|ruff|primrose|gold|pot|seed|marigold|heather|moor|sprig|heath|jaw|perch|dragon|snapdragon|cloche|ring|chime|bluebell|tower|throat|glove|foxglove|hyacinth|cluster|bell|raceme|crocus|saffron|tip|stigma|lily|pollen|crest|anther|violet|patch|moss|thicket|tulip|stem|glow|bloom|pad|ripple|pond|lotus|spur|veil|spike|orchid|blade|dew|iris|petal|drift|aster|quill|gleam|urn|zinnia|spire|ember|vase|dahlia|fizz|flare|planter|azalea|wax|rose|tray|snow|velvet|latch|claim|roselle|punch|sip|hibiscus|cream|blush|magnolia|blossom|honey|citrus|earl|saucer|daisy|tea|cup|capsule|silk|calm|bundle|nip|dream|pouch|chill|frost|tin|needle|woody|pinch|twig|jar|softleaf|dusty|peel|stone|wild|pizza|spear|bitters|cruet|frondlet|ledge|garnish|bed|gardenia|camellia|peony|bud|satin|bowl|poppy)\b/i.test([l241.name, l242.name].join(","))) {
    throw new Error("L241–L242 must not reuse Meadow/Gloss/Petal/Buttercup/Pale/Dish/Ruff/Primrose/Moor/Sprig/Heath/Gold/Pot/Seed/Heather/Marigold and prior triad names");
  }
  const cosmosGatePair = (level: typeof l241) => level.gates.map((g) => `${g.x},${g.y}`).sort().join("/");
  if (cosmosGatePair(l241) !== "3,0/3,3") throw new Error(`L241 gates must be delta (0, 3) pair, got ${cosmosGatePair(l241)}`);
  if (cosmosGatePair(l242) !== "0,3/1,3") throw new Error(`L242 gates must be delta (1, 0) pair, got ${cosmosGatePair(l242)}`);
  if (cosmosGatePair(l243) !== "2,4/5,1") throw new Error(`L243 gates must be delta (3, 3) pair, got ${cosmosGatePair(l243)}`);
  const occ243 = [...l243.cats, ...l243.gates].map((p) => `${p.x},${p.y}`).sort().join(";");
  ...
  if (priorOcc.has(occ243)) throw new Error("L243 Floret must not twin Cup/Pad/Pond/Vase/Bloom/Thicket/Anther/Stigma/Raceme/Glove/Chime/Dragon/Seed/Heath/Ruff/Petal occupancy");
  if (occ243.split(";").includes("5,5")) throw new Error("L243 Floret must be off Nori seat (5,5)");
  if (l243.gates[0].y === l243.gates[1].y) throw new Error("L243 Floret must not same-row Nest");
  if (l243.gates[0].x === l243.gates[1].x) throw new Error("L243 Floret must not column Porch");
  if (l241.cats[0].x === l241.cats[1].x) throw new Error("L241 Airy must not stacked-column Vine");

  if (solves.L241[0][0] === "cat_gray" && solves.L241[0][1] === "e") {
    throw new Error("L241 Airy Cut must not open gray-east (Pale Cut clone)");
  }
  if (solves.L241[0][0] === "cat_orange" && solves.L241[0][1] === "n") {
    throw new Error("L241 Airy Cut must not open orange-north (Moor Cut clone)");
  }
  if (solves.L241[0][0] === "cat_black" && solves.L241[0][1] === "s") {
    throw new Error("L241 Airy Cut must not open black-south (Gold Cut clone)");
  }
  if (solves.L242[0][0] === "cat_black" && solves.L242[0][1] === "s") {
    throw new Error("L242 Ray Gap must not open black-south (Dish Gap clone)");
  }
  if (solves.L242[0][0] === "cat_gray" && solves.L242[0][1] === "e") {
    throw new Error("L242 Ray Gap must not open gray-east (Sprig Gap clone)");
  }
  if (solves.L243[0][0] === "cat_orange" && solves.L243[0][1] === "s") {
    throw new Error("L243 Floret Stop must not open orange-south (Ruff Stop clone)");
  }
  if (solves.L243[0][0] === "cat_black" && solves.L243[0][1] === "s") {
    throw new Error("L243 Floret Stop must not open black-south (Heath Stop clone)");
  }
  if (solves.L241[0][0] === "cat_black" && solves.L241[0][1] === "e") {
    throw new Error("L241 Airy Cut must not open black-east (Jaw Cut clone)");
  }
  if (solves.L241[0][0] === "cat_gray" && solves.L241[0][1] === "s") {
    throw new Error("L241 Airy Cut must not open gray-south (Cloche Cut clone)");
  }
  if (solves.L241[0][0] === "cat_orange" && solves.L241[0][1] === "s") {
    throw new Error("L241 Airy Cut must not open orange-south (Tower Cut clone)");
  }
  if (solves.L241[0][0] === "cat_black" && solves.L241[0][1] === "w") {
    throw new Error("L241 Airy Cut must not open black-west (Pollen Cut clone)");
  }
  if (solves.L241[0][0] === "cat_gray" && solves.L241[0][1] === "w") {
    throw new Error("L241 Airy Cut must not open gray-west (Pad Cut clone)");
  }
  if (solves.L241[0][0] === "cat_black" && solves.L241[0][1] === "n") {
    throw new Error("L241 Airy Cut must not open black-north (Cluster Cut clone)");
  }
  if (solves.L241[0][0] === "cat_orange" && solves.L241[0][1] === "w") {
    throw new Error("L241 Airy Cut must not open orange-west (Saffron Cut clone)");
  }
  if (solves.L242[0][0] === "cat_gray" && solves.L242[0][1] === "n") {
    throw new Error("L242 Ray Gap must not open gray-north (Pot Gap clone)");
  }
  if (solves.L242[0][0] === "cat_black" && solves.L242[0][1] === "n") {
    throw new Error("L242 Ray Gap must not open black-north (Perch Gap clone)");
  }
  if (solves.L242[0][0] === "cat_orange" && solves.L242[0][1] === "w") {
    throw new Error("L242 Ray Gap must not open orange-west (Ring Gap clone)");
  }
  if (solves.L242[0][0] === "cat_gray" && solves.L242[0][1] === "s") {
    throw new Error("L242 Ray Gap must not open gray-south (Bell Gap clone)");
  }
  if (solves.L242[0][0] === "cat_black" && solves.L242[0][1] === "e") {
    throw new Error("L242 Ray Gap must not open black-east (Ripple Gap clone)");
  }
  if (solves.L242[0][0] === "cat_orange" && solves.L242[0][1] === "s") {
    throw new Error("L242 Ray Gap must not open orange-south (Veil Gap clone)");
  }
  if (solves.L242[0][0] === "cat_orange" && solves.L242[0][1] === "n") {
    throw new Error("L242 Ray Gap must not open orange-north (Dew Gap clone)");
  }
  if (solves.L242[0][0] === "cat_orange" && solves.L242[0][1] === "e") {
    throw new Error("L242 Ray Gap must not open orange-east (Tip Gap clone)");
  }
  if (solves.L243[0][0] === "cat_gray" && solves.L243[0][1] === "s") {
    throw new Error("L243 Floret Stop must not open gray-south (Seed Stop clone)");
  }
  if (solves.L243[0][0] === "cat_orange" && solves.L243[0][1] === "e") {
    throw new Error("L243 Floret Stop must not open orange-east (Dragon Stop clone)");
  }
  if (solves.L243[0][0] === "cat_orange" && solves.L243[0][1] === "n") {
    throw new Error("L243 Floret Stop must not open orange-north (Chime Stop clone)");
  }
  if (solves.L243[0][0] === "cat_black" && solves.L243[0][1] === "e") {
    throw new Error("L243 Floret Stop must not open black-east (Raceme Stop clone)");
  }
  if (solves.L243[0][0] === "cat_orange" && solves.L243[0][1] === "w") {
    throw new Error("L243 Floret Stop must not open orange-west (Spike Stop clone)");
  }
  if (solves.L243[0][0] === "cat_gray" && solves.L243[0][1] === "n") {
    throw new Error("L243 Floret Stop must not open gray-north (Dish Stop clone)");
  }
  if (solves.L243[0][0] === "cat_gray" && solves.L243[0][1] === "w") {
    throw new Error("L243 Floret Stop must not open gray-west (Urn Stop clone)");
  }
  if (solves.L243[0][0] === "cat_gray" && solves.L243[0][1] === "e") {
    throw new Error("L243 Floret Stop must not open gray-east (Petal Cut clone)");
  }
  if (solves.L241[0][0] === "cat_gray" && solves.L241[0][1] === "n") {
    throw new Error("L241 Airy Cut must not open gray-north (Meadow Cut clone)");
  }
  if (solves.L242[0][0] === "cat_black" && solves.L242[0][1] === "w") {
    throw new Error("L242 Ray Gap must not open black-west (Gloss Gap clone)");
  }
  if (solves.L243[0][0] === "cat_black" && solves.L243[0][1] === "n") {
    throw new Error("L243 Floret Stop must not open black-north (Petal Stop clone)");
  }
  for (const level of [l241, l242, l243]) {
  ...
  const wallKeyO = (level: typeof l241) =>
  ...
  for (const id of ["L241", "L242", "L243"] as const) {
    const key = wallKeyO(LEVELS.find((r) => r.id === id)!);
    if (priorWallsO.includes(key)) throw new Error(`${id} wall twin of L139–L240`);
  }
  if (ART_KIT_PATH.cosmos.loaf48 !== "/assets/cats/cosmos_loaf_48.svg") throw new Error("ART_KIT_PATH.cosmos loaf48");
  if (ART_KIT_PATH.cosmos.loaf72 !== "/assets/cats/cosmos_loaf_72.svg") throw new Error("ART_KIT_PATH.cosmos loaf72");
  ...
  if (!saveF.includes("cosmos stem")) throw new Error("SaveProvider must yard-bubble cosmos stem for Cosmos");
  ...
  if (!existsSync(resolve("data/collection/CH4_FRIEND_080.md"))) throw new Error("missing CH4_FRIEND_080.md");
  if (!existsSync(resolve("data/collection/chapter4_cosmos_bang.json"))) throw new Error("missing collection cosmos bang");
  if (!existsSync(resolve("data/chapter4_cosmos_bang.json"))) throw new Error("missing chapter4_cosmos_bang.json");
  const nameModalF = readFileSync(resolve("src/components/puzzle/NameCatModal.tsx"), "utf8");
  if (!nameModalF.includes("COSMOS_FRIEND_ID")) throw new Error("NameCatModal must lockChips Cosmos");
