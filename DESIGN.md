...
Authoritative teach boards: `data/levels/L01-L03.json` (studio handoff). L2 is a two-cell wall run (Template 01 prose said “1 wall”; the JSON is the lock). L4–L9 continue the locked Chapter 2 packs. L11–L24 are the Chapter 3 color-brake boards (budgets from `data/levels/move_budget_L01-L30.json`; L25–30 remain stubs). Engine: `src/lib/slide.ts`. Unique clears emit `onClear(clearIndex)` from `src/lib/onClear.ts`. Parade friends key off the level number so L12 awards Tux, L15 awards Ghost, L18 awards Mist, and L21 awards Pepper even though L10 is off the path.
...
Same slide engine. Campaign after L9 is **L11–L24** (skip L10).
...
- **onClear(21) → Pepper (`friend_007`) after L21.** Orange spotted loaf, chips Pepper / Spice / Pip, no furniture gift. Pack: `data/chapter3_pepper_bang.json`. Pumpkin@24 ships later.
...
First seven rescues (`TUTORIAL_RESCUES`) are a curated C/B, Regular-body parade: Mango, Ink, Biscuit, Tux, Ghost, Mist, Pepper. Naming modal reveals the personality line, suggests a name (pools include Mochi / Bean / Pip-adjacent food names), Confirm → yard drop-in + `"{Name} moved in!"` and a first-night !. Session return can show 1–3 ! bubbles.
...
Cadence is Collection v1.1: cat every 3 unique clears for the first 20, Hearts on every clear. Porch/Lawn only. Comfort on the HUD. Friends / Met tabs list named instances vs the tutorial seven.
