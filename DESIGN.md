...
Authoritative teach boards: `data/levels/L01-L03.json` (studio handoff). L2 is a two-cell wall run (Template 01 prose said “1 wall”; the JSON is the lock). L4–L9 continue the locked Chapter 2 packs. L11–L27 are the Chapter 3 color-brake boards (budgets from `data/levels/move_budget_L01-L30.json`; L28–30 remain stubs). Engine: `src/lib/slide.ts`. Unique clears emit `onClear(clearIndex)` from `src/lib/onClear.ts`. Parade friends key off the level number so L12 awards Tux, L15 awards Ghost, L18 awards Mist, L21 awards Pepper, and L24 awards Pumpkin even though L10 is off the path.
...
Same slide engine. Campaign after L9 is **L11–L27** (skip L10).
...
- **onClear(24) → Pumpkin (`friend_008`) after L24.** Orange classic loaf, chips Pumpkin / Squash / Ember, no furniture gift. Pack: `data/chapter3_pumpkin_bang.json`. Shadow@27 ships later.
...
First eight rescues (`TUTORIAL_RESCUES`) are a curated C/B, Regular-body parade: Mango, Ink, Biscuit, Tux, Ghost, Mist, Pepper, Pumpkin. Naming modal reveals the personality line, suggests a name (pools include Mochi / Bean / Pip-adjacent food names), Confirm → yard drop-in + `"{Name} moved in!"` and a first-night !. Session return can show 1–3 ! bubbles.
...
Slice dopamine beat: **Mango @ `onClear(3)`** after teach L1–L3, then naming + box gift. Unlock table covers **3 / 6 / 9 / 12 / 15 / 18 / 21 / 24**. CURRENT is frozen and correct. **NO Pebble.** Parade: Mango@3, Ink@6, Biscuit@9, Tux@12, Ghost@15, Mist@18, Pepper@21, Pumpkin@24, Shadow@27 (first full-black), Noodle@30, … Bean@60. Ignore any chat saying Pebble.
