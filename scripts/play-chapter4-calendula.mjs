/**
 * Chapter 4 beat: L282 → Calendula@282 naming, then L283–L285.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L279 queues Salvia pending. Do not invent friend_095.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-calendula";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 271 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Salvia claimed the salvia torch."],
...
try {
  ...
  await page.goto(`${BASE}/level/L282`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("BRACT STOP"));
  await shot(page, "01_l282_before_calendula");
  await play(
    page,
    [
      ["orange", "ArrowLeft"],
  ...
  console.log("calendula naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Calendula prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Petal soft. Already claimed the calendula tin.") {
    throw new Error(`Calendula display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Calendula,Petal,Tin") {
    throw new Error(`Calendula chips must be Calendula/Petal/Tin, got ${modal.chips.join("/")}`);
  }
  const bannedChips = [
    "Salvia", "Sage", "Torch", "Impatiens", "Busy", "Box", "Verbena", "Spike", "Pot", "Pansy", "Face", "Saucer", "Petunia", "Flare", "Basket",
  ...
  if (modal.chips.some((chip) => bannedChips.includes(chip))) {
    throw new Error("Calendula chips collided with Salvia/Impatiens/Verbena/Pansy and prior pools");
  }
  if (!modal.hero.some((src) => src?.includes("calendula_loaf"))) {
    throw new Error(`Calendula hero missing calendula loaf: ${modal.hero}`);
  }
  if (modal.hero.some((src) => src?.includes("salvia_loaf"))) {
    throw new Error("Calendula hero must not use the Salvia loaf");
  }
  if (modal.hero.some((src) => src?.includes("impatiens_loaf"))) {
    throw new Error("Calendula hero must not use the Impatiens loaf");
  }
  if (modal.hero.some((src) => src?.includes("verbena_loaf"))) {
    throw new Error("Calendula hero must not use the Verbena loaf");
  }
  if (modal.hero.some((src) => src?.includes("pansy_loaf"))) {
    throw new Error("Calendula hero must not use the Pansy loaf");
  }
  if (modal.hero.some((src) => src?.includes("petunia_loaf"))) {
    throw new Error("Calendula hero must not use the Petunia loaf");
  }
  if (modal.hero.some((src) => src?.includes("geranium_loaf"))) {
    throw new Error("Calendula hero must not use the Geranium loaf");
  }
  if (modal.hero.some((src) => src?.includes("begonia_loaf"))) {
    throw new Error("Calendula hero must not use the Begonia loaf");
  }
  if (modal.hero.some((src) => src?.includes("marigold_loaf"))) {
    throw new Error("Calendula hero must not use the Marigold loaf");
  }
  await shot(page, "02_calendula_naming");
  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Calendula") &&
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_094")) {
    throw new Error("Calendula was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_094")?.name !== "Calendula") {
    throw new Error("Calendula name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_095")) {
    throw new Error("friend_095 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/calendula_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/calendula_loaf_48.svg")
  ) {
    throw new Error(`yard missing Calendula loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("tin") && !yard.text.includes("Calendula")) {
    throw new Error("yard missing Calendula calendula tin line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_calendula_yard");

  await page.goto(`${BASE}/level/L283`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("DISK CUT"));
  await play(
    page,
    [
      ["orange", "ArrowUp"],
  ...
  await page.goto(`${BASE}/level/L284`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("TIN GAP"));
  await play(
    page,
    [
      ["gray", "ArrowUp"],
  ...
  for (const id of ["L282", "L283", "L284", "L285"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_095")) {
    throw new Error("friend_095 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_095")) {
    throw new Error("L285 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L285 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_094").length !== 1) {
    throw new Error("L285 must not unlock a Calendula duplicate");
  }
  if (!after.hud.includes("285") && !after.text.includes("285")) {
    throw new Error("Campaign HUD must show through 285 after L285 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L282 + CALENDULA + L283-285 OK");
} catch (error) {
  console.error("CHAPTER 4 CALENDULA FAIL", error);
  try {
