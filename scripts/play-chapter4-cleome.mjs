/**
 * Chapter 4 beat: L294 → Cleome@294 naming, then L295–L297.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L291 already named Celosia. Do not invent friend_099.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-cleome";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 283 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Celosia claimed the celosia comb."],
...
try {
  ...
  await shot(page, "01_l294_before_cleome");
  await play(
    page,
    [
      ["orange", "ArrowDown"],
  ...
  console.log("cleome naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Cleome prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Whisk soft. Already claimed the cleome pod.") {
    throw new Error(`Cleome display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Cleome,Whisk,Pod") {
    throw new Error(`Cleome chips must be Cleome/Whisk/Pod, got ${modal.chips.join("/")}`);
  }
  const bannedChips = [
    "Celosia", "Plume", "Comb", "Phlox", "Floret", "Bank", "Lantana", "Umbel", "Mound", "Calendula", "Petal", "Tin", "Salvia", "Sage", "Torch", "Impatiens", "Busy", "Box", "Verbena", "Spike", "Pot", "Pansy", "Face", "Saucer", "Petunia", "Flare", "Basket",
  ...
  if (modal.chips.some((chip) => bannedChips.includes(chip))) {
    throw new Error("Cleome chips collided with Celosia/Phlox/Lantana and prior pools");
  }
  if (!modal.hero.some((src) => src?.includes("cleome_loaf"))) {
    throw new Error(`Cleome hero missing cleome loaf: ${modal.hero}`);
  }
  if (modal.hero.some((src) => src?.includes("celosia_loaf"))) {
    throw new Error("Cleome hero must not use the Celosia loaf");
  }
  if (modal.hero.some((src) => src?.includes("phlox_loaf"))) {
    throw new Error("Cleome hero must not use the Phlox loaf");
  }
  if (modal.hero.some((src) => src?.includes("lantana_loaf"))) {
    throw new Error("Cleome hero must not use the Lantana loaf");
  }
  if (modal.hero.some((src) => src?.includes("salvia_loaf"))) {
    throw new Error("Cleome hero must not use the Salvia loaf");
  }
  await shot(page, "02_cleome_naming");
  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Cleome") &&
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_098")) {
    throw new Error("Cleome was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_098")?.name !== "Cleome") {
    throw new Error("Cleome name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_099")) {
    throw new Error("friend_099 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/cleome_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/cleome_loaf_48.svg")
  ) {
    throw new Error(`yard missing Cleome loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("pod") && !yard.text.includes("Cleome")) {
    throw new Error("yard missing Cleome cleome pod line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_cleome_yard");

  await page.goto(`${BASE}/level/L295`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("WHISK CUT"));
  await play(
    page,
    [
      ["orange", "ArrowLeft"],
  ...
  await page.goto(`${BASE}/level/L296`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("POD GAP"));
  await play(
    page,
    [
      ["gray", "ArrowDown"],
  ...
  for (const id of ["L294", "L295", "L296", "L297"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_099")) {
    throw new Error("friend_099 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_099")) {
    throw new Error("L297 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L297 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_098").length !== 1) {
    throw new Error("L297 must not unlock a Cleome duplicate");
  }
  if (!after.hud.includes("297") && !after.text.includes("297")) {
    throw new Error("Campaign HUD must show through 297 after L297 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L294 + CLEOME + L295-297 OK");
} catch (error) {
  console.error("CHAPTER 4 CLEOME FAIL", error);
  try {
