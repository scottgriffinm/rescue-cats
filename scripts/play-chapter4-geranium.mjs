/**
 * Chapter 4 beat: L261 → Geranium@261 naming, then L262–L264.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L261 queues Geranium pending. Do not invent friend_088.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-geranium";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 250 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Freesia claimed the freesia vase."],
...
try {
  ...
  await shot(page, "01_l261_before_geranium");
  await play(
    page,
    [
      ["black", "ArrowRight"],
  ...
  console.log("geranium naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Geranium prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Cluster soft. Already claimed the geranium sill.") {
    throw new Error(`Geranium display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Geranium,Cluster,Sill") {
    throw new Error(`Geranium chips must be Geranium/Cluster/Sill, got ${modal.chips.join("/")}`);
  }
  if (
    ...
    throw new Error("Geranium chips collided with Freesia/Begonia/Anemone/Wisteria/Clematis/Cosmos/Buttercup/Primrose/Heather/Marigold/Snapdragon/Bluebell/Foxglove/Hyacinth/Crocus/Lily/Violet/Tulip/Poppy/Lotus/Orchid/Iris/Aster/Zinnia/Dahlia/Azalea/Peony/Camellia/Gardenia/Hibiscus/Magnolia/Jasmine/Pebble pools");
  }
  if (!modal.hero.includes("/assets/cats/geranium_loaf_72.svg")) {
    throw new Error(`Geranium hero missing cluster loaf: ${modal.hero}`);
  }
  if (modal.hero.includes("/assets/cats/freesia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Freesia loaf");
  }
  if (modal.hero.includes("/assets/cats/begonia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Begonia loaf");
  }
  if (modal.hero.includes("/assets/cats/ranunculus_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Ranunculus loaf");
  }
  if (modal.hero.includes("/assets/cats/anemone_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Anemone loaf");
  }
  if (modal.hero.includes("/assets/cats/wisteria_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Wisteria loaf");
  }
  if (modal.hero.includes("/assets/cats/clematis_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Clematis loaf");
  }
  if (modal.hero.includes("/assets/cats/cosmos_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Cosmos loaf");
  }
  if (modal.hero.includes("/assets/cats/buttercup_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Buttercup loaf");
  }
  if (modal.hero.includes("/assets/cats/primrose_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Primrose loaf");
  }
  if (modal.hero.includes("/assets/cats/heather_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Heather loaf");
  }
  if (modal.hero.includes("/assets/cats/marigold_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Marigold loaf");
  }
  if (modal.hero.includes("/assets/cats/snapdragon_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Snapdragon loaf");
  }
  if (modal.hero.includes("/assets/cats/bluebell_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Bluebell loaf");
  }
  if (modal.hero.includes("/assets/cats/foxglove_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Foxglove loaf");
  }
  if (modal.hero.includes("/assets/cats/hyacinth_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Hyacinth loaf");
  }
  if (modal.hero.includes("/assets/cats/crocus_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Crocus loaf");
  }
  if (modal.hero.includes("/assets/cats/lily_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Lily loaf");
  }
  if (modal.hero.includes("/assets/cats/violet_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Violet loaf");
  }
  if (modal.hero.includes("/assets/cats/tulip_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Tulip loaf");
  }
  if (modal.hero.includes("/assets/cats/poppy_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Poppy loaf");
  }
  if (modal.hero.includes("/assets/cats/lotus_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Lotus loaf");
  }
  if (modal.hero.includes("/assets/cats/orchid_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Orchid loaf");
  }
  if (modal.hero.includes("/assets/cats/iris_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Iris loaf");
  }
  if (modal.hero.includes("/assets/cats/aster_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Aster loaf");
  }
  if (modal.hero.includes("/assets/cats/zinnia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Zinnia loaf");
  }
  if (modal.hero.includes("/assets/cats/dahlia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Dahlia loaf");
  }
  if (modal.hero.includes("/assets/cats/azalea_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Azalea loaf");
  }
  if (modal.hero.includes("/assets/cats/peony_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Peony loaf");
  }
  if (modal.hero.includes("/assets/cats/camellia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Camellia loaf");
  }
  if (modal.hero.includes("/assets/cats/gardenia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Gardenia loaf");
  }
  if (modal.hero.includes("/assets/cats/hibiscus_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Hibiscus loaf");
  }
  if (modal.hero.includes("/assets/cats/jasmine_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Jasmine loaf");
  }
  if (modal.hero.includes("/assets/cats/magnolia_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Magnolia loaf");
  }
  if (modal.hero.includes("/assets/cats/lavender_loaf_72.svg")) {
    throw new Error("Geranium hero must not use the Lavender loaf");
  }
  await shot(page, "02_geranium_naming");

  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Geranium") &&
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_087")) {
    throw new Error("Geranium was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_087")?.name !== "Geranium") {
    throw new Error("Geranium name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_088")) {
    throw new Error("friend_088 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/geranium_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/geranium_loaf_48.svg")
  ) {
    throw new Error(`yard missing Geranium cluster loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("sill") && !yard.text.includes("Geranium")) {
    throw new Error("yard missing Geranium geranium sill line");
  }
  ...
  await page.goto(`${BASE}/level/L263`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("SILL GAP"));
  await play(
    page,
    [
      ["black", "ArrowDown"],
  ...
  for (const id of ["L261", "L262", "L263", "L264"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_088")) {
    throw new Error("friend_088 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_088")) {
    throw new Error("L264 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L264 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_087").length !== 1) {
    throw new Error("L264 must not unlock a Geranium duplicate");
  }
  if (!after.hud.includes("264") && !after.text.includes("264")) {
    throw new Error("Campaign HUD must show through 264 after L264 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L261 + GERANIUM + L262-264 OK");
} catch (error) {
  console.error("CHAPTER 4 GERANIUM FAIL", error);
  try {
