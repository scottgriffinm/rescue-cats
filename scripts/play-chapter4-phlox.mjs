/**
 * Chapter 4 beat: L288 → Phlox@288 naming, then L289–L291.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L285 queues Lantana pending. Do not invent friend_097.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-phlox";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 277 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Lantana claimed the lantana mound."],
...
try {
  ...
  await shot(page, "01_l288_before_phlox");
  await play(
    page,
    [
      ["orange", "ArrowLeft"],
  ...
  console.log("phlox naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Phlox prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Floret soft. Already claimed the phlox bank.") {
    throw new Error(`Phlox display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Phlox,Floret,Bank") {
    throw new Error(`Phlox chips must be Phlox/Floret/Bank, got ${modal.chips.join("/")}`);
  }
  const bannedChips = [
    "Lantana", "Umbel", "Mound", "Calendula", "Petal", "Tin", "Salvia", "Sage", "Torch", "Impatiens", "Busy", "Box", "Verbena", "Spike", "Pot", "Pansy", "Face", "Saucer", "Petunia", "Flare", "Basket",
  ...
  if (modal.chips.some((chip) => bannedChips.includes(chip))) {
    throw new Error("Phlox chips collided with Lantana/Calendula/Salvia and prior pools");
  }
  if (!modal.hero.some((src) => src?.includes("phlox_loaf"))) {
    throw new Error(`Phlox hero missing phlox loaf: ${modal.hero}`);
  }
  if (modal.hero.some((src) => src?.includes("lantana_loaf"))) {
    throw new Error("Phlox hero must not use the Lantana loaf");
  }
  if (modal.hero.some((src) => src?.includes("calendula_loaf"))) {
    throw new Error("Phlox hero must not use the Calendula loaf");
  }
  if (modal.hero.some((src) => src?.includes("impatiens_loaf"))) {
    throw new Error("Phlox hero must not use the Impatiens loaf");
  }
  if (modal.hero.some((src) => src?.includes("petunia_loaf"))) {
    throw new Error("Phlox hero must not use the Petunia loaf");
  }
  if (modal.hero.some((src) => src?.includes("pansy_loaf"))) {
    throw new Error("Phlox hero must not use the Pansy loaf");
  }
  await shot(page, "02_phlox_naming");
  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Phlox") &&
      ((document.body.innerText || "").includes("bank") ||
        (document.body.innerText || "").includes("phlox") ||
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_096")) {
    throw new Error("Phlox was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_096")?.name !== "Phlox") {
    throw new Error("Phlox name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_097")) {
    throw new Error("friend_097 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/phlox_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/phlox_loaf_48.svg")
  ) {
    throw new Error(`yard missing Phlox loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("bank") && !yard.text.includes("Phlox")) {
    throw new Error("yard missing Phlox phlox bank line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_phlox_yard");

  await page.goto(`${BASE}/level/L289`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("FLORET CUT"));
  await play(
    page,
    [
      ["orange", "ArrowRight"],
  ...
  await page.goto(`${BASE}/level/L290`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("BANK GAP"));
  await play(
    page,
    [
      ["gray", "ArrowRight"],
  ...
  for (const id of ["L288", "L289", "L290", "L291"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_097")) {
    throw new Error("friend_097 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_097")) {
    throw new Error("L291 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L291 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_096").length !== 1) {
    throw new Error("L291 must not unlock a Phlox duplicate");
  }
  if (!after.hud.includes("291") && !after.text.includes("291")) {
    throw new Error("Campaign HUD must show through 291 after L291 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L288 + PHLOX + L289-291 OK");
} catch (error) {
  console.error("CHAPTER 4 PHLOX FAIL", error);
  try {
