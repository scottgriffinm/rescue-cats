/**
 * Chapter 4 beat: L264 → Nasturtium@264 naming, then L265–L267.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L264 queues Nasturtium pending. Do not invent friend_089.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-nasturtium";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 253 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Geranium claimed the geranium sill."],
...
try {
  ...
  await page.goto(`${BASE}/level/L264`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("PANE STOP"));
  await shot(page, "01_l264_before_nasturtium");
  await play(
    page,
    [
      ["orange", "ArrowRight"],
  ...
  console.log("nasturtium naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Nasturtium prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Pepper soft. Already claimed the nasturtium tray.") {
    throw new Error(`Nasturtium display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Nasturtium,Pepper,Tray") {
    throw new Error(`Nasturtium chips must be Nasturtium/Pepper/Tray, got ${modal.chips.join("/")}`);
  }
  if (
    ...
    throw new Error("Nasturtium chips collided with  pools");
  }
  if (!modal.hero.includes("/assets/cats/nasturtium_loaf_72.svg")) {
    throw new Error(`Nasturtium hero missing pepper loaf: ${modal.hero}`);
  }
  if (modal.hero.includes("/assets/cats/geranium_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Geranium loaf");
  }
  if (modal.hero.includes("/assets/cats/freesia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Freesia loaf");
  }
  if (modal.hero.includes("/assets/cats/begonia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Begonia loaf");
  }
  if (modal.hero.includes("/assets/cats/ranunculus_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Ranunculus loaf");
  }
  if (modal.hero.includes("/assets/cats/anemone_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Anemone loaf");
  }
  if (modal.hero.includes("/assets/cats/wisteria_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Wisteria loaf");
  }
  if (modal.hero.includes("/assets/cats/clematis_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Clematis loaf");
  }
  if (modal.hero.includes("/assets/cats/cosmos_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Cosmos loaf");
  }
  if (modal.hero.includes("/assets/cats/buttercup_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Buttercup loaf");
  }
  if (modal.hero.includes("/assets/cats/primrose_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Primrose loaf");
  }
  if (modal.hero.includes("/assets/cats/heather_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Heather loaf");
  }
  if (modal.hero.includes("/assets/cats/marigold_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Marigold loaf");
  }
  if (modal.hero.includes("/assets/cats/snapdragon_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Snapdragon loaf");
  }
  if (modal.hero.includes("/assets/cats/bluebell_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Bluebell loaf");
  }
  if (modal.hero.includes("/assets/cats/foxglove_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Foxglove loaf");
  }
  if (modal.hero.includes("/assets/cats/hyacinth_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Hyacinth loaf");
  }
  if (modal.hero.includes("/assets/cats/crocus_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Crocus loaf");
  }
  if (modal.hero.includes("/assets/cats/lily_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Lily loaf");
  }
  if (modal.hero.includes("/assets/cats/violet_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Violet loaf");
  }
  if (modal.hero.includes("/assets/cats/tulip_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Tulip loaf");
  }
  if (modal.hero.includes("/assets/cats/poppy_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Poppy loaf");
  }
  if (modal.hero.includes("/assets/cats/lotus_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Lotus loaf");
  }
  if (modal.hero.includes("/assets/cats/orchid_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Orchid loaf");
  }
  if (modal.hero.includes("/assets/cats/iris_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Iris loaf");
  }
  if (modal.hero.includes("/assets/cats/aster_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Aster loaf");
  }
  if (modal.hero.includes("/assets/cats/zinnia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Zinnia loaf");
  }
  if (modal.hero.includes("/assets/cats/dahlia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Dahlia loaf");
  }
  if (modal.hero.includes("/assets/cats/azalea_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Azalea loaf");
  }
  if (modal.hero.includes("/assets/cats/peony_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Peony loaf");
  }
  if (modal.hero.includes("/assets/cats/camellia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Camellia loaf");
  }
  if (modal.hero.includes("/assets/cats/gardenia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Gardenia loaf");
  }
  if (modal.hero.includes("/assets/cats/hibiscus_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Hibiscus loaf");
  }
  if (modal.hero.includes("/assets/cats/jasmine_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Jasmine loaf");
  }
  if (modal.hero.includes("/assets/cats/magnolia_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Magnolia loaf");
  }
  if (modal.hero.includes("/assets/cats/lavender_loaf_72.svg")) {
    throw new Error("Nasturtium hero must not use the Lavender loaf");
  }
  await shot(page, "02_nasturtium_naming");

  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Nasturtium") &&
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_088")) {
    throw new Error("Nasturtium was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_088")?.name !== "Nasturtium") {
    throw new Error("Nasturtium name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_089")) {
    throw new Error("friend_089 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/nasturtium_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/nasturtium_loaf_48.svg")
  ) {
    throw new Error(`yard missing Nasturtium pepper loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("tray") && !yard.text.includes("Nasturtium")) {
    throw new Error("yard missing Nasturtium nasturtium tray line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_nasturtium_yard");

  await page.goto(`${BASE}/level/L265`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("PEPPER CUT"));
  await play(
    page,
    [
      ["orange", "ArrowDown"],
  ...
  await page.goto(`${BASE}/level/L266`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("TRAY GAP"));
  await play(
    page,
    [
      ["gray", "ArrowRight"],
  ...
  await shot(page, "04_l266_win");

  await page.goto(`${BASE}/level/L267`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("TENDRIL STOP"));
  await play(
    page,
    [
      ["black", "ArrowUp"],
  ...
  for (const id of ["L264", "L265", "L266", "L267"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_089")) {
    throw new Error("friend_089 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_089")) {
    throw new Error("L267 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L267 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_088").length !== 1) {
    throw new Error("L267 must not unlock a Nasturtium duplicate");
  }
  if (!after.hud.includes("267") && !after.text.includes("267")) {
    throw new Error("Campaign HUD must show through 267 after L267 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L264 + NASTURTIUM + L265-267 OK");
} catch (error) {
  console.error("CHAPTER 4 NASTURTIUM FAIL", error);
  try {
