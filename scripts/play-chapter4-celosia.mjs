/**
 * Chapter 4 beat: L291 → Celosia@291 naming, then L292–L294.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L288 already named Phlox. Do not invent friend_098.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-celosia";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 280 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Phlox claimed the phlox bank."],
...
try {
  ...
  await page.goto(`${BASE}/level/L291`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("SPRAY STOP"));
  await shot(page, "01_l291_before_celosia");
  await play(
    page,
    [
      ["black", "ArrowRight"],
  ...
  console.log("celosia naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Celosia prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Plume soft. Already claimed the celosia comb.") {
    throw new Error(`Celosia display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Celosia,Plume,Comb") {
    throw new Error(`Celosia chips must be Celosia/Plume/Comb, got ${modal.chips.join("/")}`);
  }
  const bannedChips = [
    "Phlox", "Floret", "Bank", "Lantana", "Umbel", "Mound", "Calendula", "Petal", "Tin", "Salvia", "Sage", "Torch", "Impatiens", "Busy", "Box", "Verbena", "Spike", "Pot", "Pansy", "Face", "Saucer", "Petunia", "Flare", "Basket",
  ...
  if (modal.chips.some((chip) => bannedChips.includes(chip))) {
    throw new Error("Celosia chips collided with Phlox/Lantana/Calendula and prior pools");
  }
  if (!modal.hero.some((src) => src?.includes("celosia_loaf"))) {
    throw new Error(`Celosia hero missing celosia loaf: ${modal.hero}`);
  }
  if (modal.hero.some((src) => src?.includes("phlox_loaf"))) {
    throw new Error("Celosia hero must not use the Phlox loaf");
  }
  if (modal.hero.some((src) => src?.includes("lantana_loaf"))) {
    throw new Error("Celosia hero must not use the Lantana loaf");
  }
  if (modal.hero.some((src) => src?.includes("calendula_loaf"))) {
    throw new Error("Celosia hero must not use the Calendula loaf");
  }
  if (modal.hero.some((src) => src?.includes("impatiens_loaf"))) {
    throw new Error("Celosia hero must not use the Impatiens loaf");
  }
  if (modal.hero.some((src) => src?.includes("salvia_loaf"))) {
    throw new Error("Celosia hero must not use the Salvia loaf");
  }
  await shot(page, "02_celosia_naming");
  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Celosia") &&
      ((document.body.innerText || "").includes("comb") ||
        (document.body.innerText || "").includes("celosia") ||
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_097")) {
    throw new Error("Celosia was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_097")?.name !== "Celosia") {
    throw new Error("Celosia name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_098")) {
    throw new Error("friend_098 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/celosia_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/celosia_loaf_48.svg")
  ) {
    throw new Error(`yard missing Celosia loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("comb") && !yard.text.includes("Celosia")) {
    throw new Error("yard missing Celosia celosia comb line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_celosia_yard");

  await page.goto(`${BASE}/level/L292`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("PLUME CUT"));
  await play(
    page,
    [
      ["gray", "ArrowUp"],
  ...
  await page.goto(`${BASE}/level/L293`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("COMB GAP"));
  await play(
    page,
    [
      ["black", "ArrowDown"],
  ...
  await shot(page, "04_l293_win");

  await page.goto(`${BASE}/level/L294`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("FLAME STOP"));
  await play(
    page,
    [
      ["orange", "ArrowDown"],
  ...
  for (const id of ["L291", "L292", "L293", "L294"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_098")) {
    throw new Error("friend_098 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_098")) {
    throw new Error("L294 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L294 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_097").length !== 1) {
    throw new Error("L294 must not unlock a Celosia duplicate");
  }
  if (!after.hud.includes("294") && !after.text.includes("294")) {
    throw new Error("Campaign HUD must show through 294 after L294 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L291 + CELOSIA + L292-294 OK");
} catch (error) {
  console.error("CHAPTER 4 CELOSIA FAIL", error);
  try {
