/**
 * Chapter 4 beat: L267 → Petunia@267 naming, then L268–L270.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L267 queues Petunia pending. Do not invent friend_090.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-petunia";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 256 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Nasturtium claimed the nasturtium tray."],
...
try {
  ...
  await shot(page, "01_l267_before_petunia");
  await play(
    page,
    [
      ["black", "ArrowUp"],
  ...
  console.log("petunia naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Petunia prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Flare soft. Already claimed the petunia basket.") {
    throw new Error(`Petunia display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Petunia,Flare,Basket") {
    throw new Error(`Petunia chips must be Petunia/Flare/Basket, got ${modal.chips.join("/")}`);
  }
  if (
    ...
    throw new Error("Petunia chips collided with  pools");
  }
  if (!modal.hero.includes("/assets/cats/petunia_loaf_72.svg")) {
    throw new Error(`Petunia hero missing trumpet-flare loaf: ${modal.hero}`);
  }
  if (modal.hero.includes("/assets/cats/nasturtium_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Nasturtium loaf");
  }
  if (modal.hero.includes("/assets/cats/geranium_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Geranium loaf");
  }
  if (modal.hero.includes("/assets/cats/freesia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Freesia loaf");
  }
  if (modal.hero.includes("/assets/cats/begonia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Begonia loaf");
  }
  if (modal.hero.includes("/assets/cats/ranunculus_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Ranunculus loaf");
  }
  if (modal.hero.includes("/assets/cats/anemone_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Anemone loaf");
  }
  if (modal.hero.includes("/assets/cats/wisteria_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Wisteria loaf");
  }
  if (modal.hero.includes("/assets/cats/clematis_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Clematis loaf");
  }
  if (modal.hero.includes("/assets/cats/cosmos_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Cosmos loaf");
  }
  if (modal.hero.includes("/assets/cats/buttercup_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Buttercup loaf");
  }
  if (modal.hero.includes("/assets/cats/primrose_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Primrose loaf");
  }
  if (modal.hero.includes("/assets/cats/heather_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Heather loaf");
  }
  if (modal.hero.includes("/assets/cats/marigold_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Marigold loaf");
  }
  if (modal.hero.includes("/assets/cats/snapdragon_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Snapdragon loaf");
  }
  if (modal.hero.includes("/assets/cats/bluebell_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Bluebell loaf");
  }
  if (modal.hero.includes("/assets/cats/foxglove_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Foxglove loaf");
  }
  if (modal.hero.includes("/assets/cats/hyacinth_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Hyacinth loaf");
  }
  if (modal.hero.includes("/assets/cats/crocus_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Crocus loaf");
  }
  if (modal.hero.includes("/assets/cats/lily_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Lily loaf");
  }
  if (modal.hero.includes("/assets/cats/violet_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Violet loaf");
  }
  if (modal.hero.includes("/assets/cats/tulip_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Tulip loaf");
  }
  if (modal.hero.includes("/assets/cats/poppy_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Poppy loaf");
  }
  if (modal.hero.includes("/assets/cats/lotus_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Lotus loaf");
  }
  if (modal.hero.includes("/assets/cats/orchid_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Orchid loaf");
  }
  if (modal.hero.includes("/assets/cats/iris_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Iris loaf");
  }
  if (modal.hero.includes("/assets/cats/aster_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Aster loaf");
  }
  if (modal.hero.includes("/assets/cats/zinnia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Zinnia loaf");
  }
  if (modal.hero.includes("/assets/cats/dahlia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Dahlia loaf");
  }
  if (modal.hero.includes("/assets/cats/azalea_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Azalea loaf");
  }
  if (modal.hero.includes("/assets/cats/peony_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Peony loaf");
  }
  if (modal.hero.includes("/assets/cats/camellia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Camellia loaf");
  }
  if (modal.hero.includes("/assets/cats/gardenia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Gardenia loaf");
  }
  if (modal.hero.includes("/assets/cats/hibiscus_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Hibiscus loaf");
  }
  if (modal.hero.includes("/assets/cats/jasmine_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Jasmine loaf");
  }
  if (modal.hero.includes("/assets/cats/magnolia_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Magnolia loaf");
  }
  if (modal.hero.includes("/assets/cats/lavender_loaf_72.svg")) {
    throw new Error("Petunia hero must not use the Lavender loaf");
  }
  await shot(page, "02_petunia_naming");

  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Petunia") &&
      ((document.body.innerText || "").includes("basket") ||
        (document.body.innerText || "").includes("petunia") ||
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_089")) {
    throw new Error("Petunia was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_089")?.name !== "Petunia") {
    throw new Error("Petunia name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_090")) {
    throw new Error("friend_090 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/petunia_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/petunia_loaf_48.svg")
  ) {
    throw new Error(`yard missing Petunia trumpet-flare loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("basket") && !yard.text.includes("Petunia")) {
    throw new Error("yard missing Petunia petunia basket line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_petunia_yard");

  await page.goto(`${BASE}/level/L268`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("FLARE CUT"));
  await play(
    page,
    [
      ["gray", "ArrowLeft"],
  ...
  await page.goto(`${BASE}/level/L269`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("BASKET GAP"));
  await play(
    page,
    [
      ["black", "ArrowUp"],
  ...
  for (const id of ["L267", "L268", "L269", "L270"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_090")) {
    throw new Error("friend_090 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_090")) {
    throw new Error("L270 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L270 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_089").length !== 1) {
    throw new Error("L270 must not unlock a Petunia duplicate");
  }
  if (!after.hud.includes("270") && !after.text.includes("270")) {
    throw new Error("Campaign HUD must show through 270 after L270 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L267 + PETUNIA + L268-270 OK");
} catch (error) {
  console.error("CHAPTER 4 PETUNIA FAIL", error);
  try {
