/**
 * Chapter 4 beat: L279 → Salvia@279 naming, then L280–L282.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L276 queues Impatiens pending. Do not invent friend_094.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-salvia";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 268 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Impatiens claimed the impatiens box."],
...
try {
  ...
  await shot(page, "01_l279_before_salvia");
  await play(
    page,
    [
      ["black", "ArrowDown"],
  ...
  console.log("salvia naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Salvia prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Sage soft. Already claimed the salvia torch.") {
    throw new Error(`Salvia display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Salvia,Sage,Torch") {
    throw new Error(`Salvia chips must be Salvia/Sage/Torch, got ${modal.chips.join("/")}`);
  }
  const bannedChips = [
    "Impatiens", "Busy", "Box", "Verbena", "Spike", "Pot", "Pansy", "Face", "Saucer", "Petunia", "Flare", "Basket",
  ...
  if (modal.chips.some((chip) => bannedChips.includes(chip))) {
    throw new Error("Salvia chips collided with Impatiens/Verbena/Pansy and prior pools");
  }
  if (!modal.hero.some((src) => src?.includes("salvia_loaf"))) {
    throw new Error(`Salvia hero missing sage loaf: ${modal.hero}`);
  }
  if (modal.hero.some((src) => src?.includes("impatiens_loaf"))) {
    throw new Error("Salvia hero must not use the Impatiens loaf");
  }
  if (modal.hero.some((src) => src?.includes("verbena_loaf"))) {
    throw new Error("Salvia hero must not use the Verbena loaf");
  }
  if (modal.hero.some((src) => src?.includes("pansy_loaf"))) {
    throw new Error("Salvia hero must not use the Pansy loaf");
  }
  if (modal.hero.some((src) => src?.includes("petunia_loaf"))) {
    throw new Error("Salvia hero must not use the Petunia loaf");
  }
  if (modal.hero.some((src) => src?.includes("geranium_loaf"))) {
    throw new Error("Salvia hero must not use the Geranium loaf");
  }
  if (modal.hero.some((src) => src?.includes("begonia_loaf"))) {
    throw new Error("Salvia hero must not use the Begonia loaf");
  }
  if (modal.hero.some((src) => src?.includes("anemone_loaf"))) {
    throw new Error("Salvia hero must not use the Anemone loaf");
  }
  await shot(page, "02_salvia_naming");
  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Salvia") &&
      ((document.body.innerText || "").includes("torch") ||
        (document.body.innerText || "").includes("salvia") ||
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_093")) {
    throw new Error("Salvia was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_093")?.name !== "Salvia") {
    throw new Error("Salvia name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_094")) {
    throw new Error("friend_094 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/salvia_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/salvia_loaf_48.svg")
  ) {
    throw new Error(`yard missing Salvia sage loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("torch") && !yard.text.includes("Salvia")) {
    throw new Error("yard missing Salvia salvia torch line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_salvia_yard");

  await page.goto(`${BASE}/level/L280`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("SAGE CUT"));
  await play(
    page,
    [
      ["gray", "ArrowDown"],
  ...
  await page.goto(`${BASE}/level/L281`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("TORCH GAP"));
  await play(
    page,
    [
      ["black", "ArrowLeft"],
  ...
  for (const id of ["L279", "L280", "L281", "L282"]) {
  ...
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_094")) {
    throw new Error("L282 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L282 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_093").length !== 1) {
    throw new Error("L282 must not unlock a Salvia duplicate");
  }
  if (!after.hud.includes("282") && !after.text.includes("282")) {
    throw new Error("Campaign HUD must show through 282 after L282 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L279 + SALVIA + L280-282 OK");
} catch (error) {
  console.error("CHAPTER 4 SALVIA FAIL", error);
  try {
