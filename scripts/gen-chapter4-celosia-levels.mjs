/**
 * Search unique 6×6 LT02+LT08 boards for L292–L294.
 * Run: npx tsx scripts/gen-chapter4-celosia-levels.mjs
 */
...
function search(spec) {
  const all = cells();
  for (let seed = spec.seed; seed < spec.seed + spec.window; seed++) {
...
for (const job of jobs) {
  ...
  if (!hit && job.requireDir) {
    ...
    hit = search({ ...job, requireDir: undefined, seed: job.seed + 90000, window: 1500000 });
  }
  if (!hit) {
...
const pack = {
  ...
  status: "SHIP READY",
  source: "Phlox Pass — L292–L294 need",
  chapter: 4,
  title: "Chapter 4 boards L292–L294",
  notes:
    "L292 Plume Cut / L293 Comb Gap / L294 Flame Stop — habit-break vs Floret Cut orange-first / Bank Gap gray-first / Spray Stop black-first; not Floret/Bank/Spray/Umbel/Mound/Bunch/Disk/Tin/Disc; Plume Cut gray-first (not Floret orange); Comb Gap black-first (not Bank gray); Flame Stop orange-first (not Spray black); no WRAP/edge twin of Nori/Medlar; L294 occupancy off prior Stop twins and Nori (5,5).",
  levels: found.map((hit) => hit.level),
  teaches: {
    plume_cut_gray: "Gray first; orange cuts after the plume slide.",
    comb_gap_black: "Black first then orange threads the comb gap.",
    flame_stop_orange: "Orange first; black seats the flame.",
  },
  solver: {
...
writeFileSync(
  "data/levels/CHAPTER4_PUZZLE_L292_L294.json",
...
console.log("wrote L292–L294 JSON + /tmp/celosia-solves.json");
