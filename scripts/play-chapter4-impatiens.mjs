/**
 * Chapter 4 beat: L276 → Impatiens@276 naming, then L277–L279.
 * Next friend must not unlock. L10 stays off-path. localStorage only.
 * L273 queues Verbena pending. Do not invent friend_093.
 */
...
const OUT = process.env.PLAY_OUT ?? "/tmp/chapter4-impatiens";
...
const completedIds = [
  ...
  "L8",
  "L9",
  ...Array.from({ length: 265 }, (_, i) => `L${i + 11}`),
];
...
const SEED = {
  ...
  levelStrikes: {},
  seenCoach: true,
  bubbles: ["Verbena claimed the verbena pot."],
...
try {
  ...
  await shot(page, "01_l276_before_impatiens");
  await play(
    page,
    [
      ["orange", "ArrowRight"],
  ...
  console.log("impatiens naming", modal);
  if (modal.title !== "New friend!") throw new Error(`title ${modal.title}`);
  if (modal.input !== "") throw new Error(`Impatiens prefilled ${modal.input}`);
  if (!modal.ctaDisabled) throw new Error("Welcome home should stay disabled");
  if (modal.line !== "Busy soft. Already claimed the impatiens box.") {
    throw new Error(`Impatiens display line drifted: ${modal.line}`);
  }
  if (modal.chips.join(",") !== "Impatiens,Busy,Box") {
    throw new Error(`Impatiens chips must be Impatiens/Busy/Box, got ${modal.chips.join("/")}`);
  }
  const bannedChips = [
    "Verbena", "Spike", "Pot", "Pansy", "Face", "Saucer", "Petunia", "Flare", "Basket",
    ...
    "Iris", "Blade", "Dew", "Aster", "Petal", "Drift", "Zinnia", "Quill", "Gleam",
  ...
  if (modal.chips.some((chip) => bannedChips.includes(chip))) {
    throw new Error("Impatiens chips collided with Verbena/Pansy/Petunia and prior pools");
  }
  if (!modal.hero.some((src) => src?.includes("impatiens_loaf"))) {
    throw new Error(`Impatiens hero missing busy loaf: ${modal.hero}`);
  }
  if (modal.hero.some((src) => src?.includes("verbena_loaf"))) {
    throw new Error("Impatiens hero must not use the Verbena loaf");
  }
  if (modal.hero.some((src) => src?.includes("pansy_loaf"))) {
    throw new Error("Impatiens hero must not use the Pansy loaf");
  }
  if (modal.hero.some((src) => src?.includes("petunia_loaf"))) {
    throw new Error("Impatiens hero must not use the Petunia loaf");
  }
  if (modal.hero.some((src) => src?.includes("geranium_loaf"))) {
    throw new Error("Impatiens hero must not use the Geranium loaf");
  }
  if (modal.hero.some((src) => src?.includes("begonia_loaf"))) {
    throw new Error("Impatiens hero must not use the Begonia loaf");
  }
  if (modal.hero.some((src) => src?.includes("anemone_loaf"))) {
    throw new Error("Impatiens hero must not use the Anemone loaf");
  }
  await shot(page, "02_impatiens_naming");
  await page.click('[aria-label="Name suggestions"] button');
  await page.click('button[type="submit"]');
  await page.waitForFunction(
    () =>
      (document.body.innerText || "").includes("Impatiens") &&
      ((document.body.innerText || "").includes("box") ||
        (document.body.innerText || "").includes("impatiens") ||
  ...
  if (!yard.save.friends.some((friend) => friend.friendId === "friend_092")) {
    throw new Error("Impatiens was not named");
  }
  if (yard.save.friends.find((friend) => friend.friendId === "friend_092")?.name !== "Impatiens") {
    throw new Error("Impatiens name was not kept");
  }
  if (yard.save.friends.some((friend) => friend.friendId === "friend_093")) {
    throw new Error("friend_093 must not unlock");
  }
  if (
    !yard.imgs.includes("/assets/cats/impatiens_loaf_72.svg") &&
    !yard.imgs.includes("/assets/cats/impatiens_loaf_48.svg")
  ) {
    throw new Error(`yard missing Impatiens busy loaf: ${yard.imgs.filter((src) => src?.includes("loaf")).join(",")}`);
  }
  if (!yard.text.includes("box") && !yard.text.includes("Impatiens")) {
    throw new Error("yard missing Impatiens impatiens box line");
  }
  if (yard.phoneFrame) throw new Error("GameShell must stay full-viewport");
  await shot(page, "03_impatiens_yard");

  await page.goto(`${BASE}/level/L277`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("BUSY CUT"));
  await play(
    page,
    [
      ["orange", "ArrowDown"],
  ...
  await page.goto(`${BASE}/level/L278`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("BOX GAP"));
  await play(
    page,
    [
      ["gray", "ArrowRight"],
  ...
  await shot(page, "04_l278_win");

  await page.goto(`${BASE}/level/L279`, { waitUntil: "networkidle0" });
  await page.waitForFunction(() => document.body.innerText.includes("TOUCH STOP"));
  await play(
    page,
    [
      ["black", "ArrowDown"],
  ...
  for (const id of ["L276", "L277", "L278", "L279"]) {
  ...
  if (after.save.friends.some((friend) => friend.friendId === "friend_093")) {
    throw new Error("friend_093 must not unlock this slice");
  }
  if (after.save.pendingUnlocks.some((pending) => pending.friendId === "friend_093")) {
    throw new Error("L279 clear must not queue the next friend");
  }
  if (after.text.includes("New friend!")) {
    throw new Error("L279 must not open a next-friend naming modal");
  }
  if (after.save.friends.filter((friend) => friend.friendId === "friend_092").length !== 1) {
    throw new Error("L279 must not unlock an Impatiens duplicate");
  }
  if (!after.hud.includes("279") && !after.text.includes("279")) {
    throw new Error("Campaign HUD must show through 279 after L279 clear");
  }
  const paradeLocks = {
    friend_001: 3,
    friend_004: 12,
  ...
  console.log("CHAPTER 4 L276 + IMPATIENS + L277-279 OK");
} catch (error) {
  console.error("CHAPTER 4 IMPATIENS FAIL", error);
  try {
